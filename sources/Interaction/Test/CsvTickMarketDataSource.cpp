 ...
 public:
  explicit CsvTickMarketDataSource(size_t index,
      ...
      : Base(index, context, instanceName, conf),
        ...
        m_halfOfBidAskSpread(conf.ReadTypedKey<double>("bid_ask_spread") / 2) {
    GetLog().Info("Source is %1%, bid-ask spread is %2%.", m_filePath,
  ...
  virtual void Run() override {
    ...
    while (std::getline(file, line)) {
      ...
      const pt::ptime time(
          ...
          pt::duration_from_string(fields[3].substr(0, 2) + ":" +
                                   fields[3].substr(2, 2) + ":" +
                                   fields[3].substr(4, 2)));
      const auto price =
          m_security->ScalePrice(boost::lexical_cast<double>(fields[4]));
      const double qty = boost::lexical_cast<double>(fields[5]);
      fields.clear();

      GetContext().SetCurrentTime(time, true);

      auto spread = m_security->ScalePrice(m_halfOfBidAskSpread);
      auto bidSpread = spread;
      auto askSpread = spread;
      if (!IsEqual(m_security->DescalePrice(spread), m_halfOfBidAskSpread)) {
        spread = 0;
        if (price < prevPrice) {
          bidSpread = 1;
          askSpread = 0;
      ...
      m_security->SetLevel1(
          time,
          Level1TickValue::Create<LEVEL1_TICK_BID_PRICE>(price - bidSpread),
          Level1TickValue::Create<LEVEL1_TICK_BID_QTY>(qty),
          Level1TickValue::Create<LEVEL1_TICK_ASK_PRICE>(price + askSpread),
          Level1TickValue::Create<LEVEL1_TICK_ASK_QTY>(qty),
          TimeMeasurement::Milestones());
      m_security->AddTrade(time, price, qty, TimeMeasurement::Milestones(),
                           true);
