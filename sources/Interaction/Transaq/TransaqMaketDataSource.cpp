...
Transaq::MarketDataSource::MarketDataSource(size_t index,
                                            Context &context,
...
void Transaq::MarketDataSource::OnReconnect() { SubscribeToSecurities(); }
...
void Transaq::MarketDataSource::SubscribeToSecurities() {
  ...
  for (const auto &security : m_securities) {
    GetLog().Debug("Sending market data subscription request for %1%...",
                   *security.second);
  ...
  try {
    ...
    GetLog().Error("Failed to send market data subscription request: \"%1%\".",
                   ex.what());
...
void Transaq::MarketDataSource::OnNewTick(const pt::ptime &time,
  ...
  security.AddTrade(time, security.ScalePrice(price), qty, delayMeasurement,
                    true);
}
...
void Transaq::MarketDataSource::OnLevel1Update(
  ...
  ticks.reserve(4);
  if (bidPrice) {
    ticks.emplace_back(Level1TickValue::Create<LEVEL1_TICK_BID_PRICE>(
        security.ScalePrice(*bidPrice)));
  }
  if (bidQty) {
    ticks.emplace_back(Level1TickValue::Create<LEVEL1_TICK_BID_QTY>(
        security.ScalePrice(*bidQty)));
  }
  if (askPrice) {
    ticks.emplace_back(Level1TickValue::Create<LEVEL1_TICK_ASK_PRICE>(
        security.ScalePrice(*askPrice)));
  }
  if (askQty) {
    ticks.emplace_back(Level1TickValue::Create<LEVEL1_TICK_ASK_QTY>(
...
Transaq::Security &Transaq::MarketDataSource::GetSecurity(
    const std::string &board, const std::string &symbol) {
...
boost::shared_ptr<trdk::MarketDataSource> CreateMarketDataSource(
  ...
  return boost::make_shared<MarketDataSource>(index, context, instanceName);
}
