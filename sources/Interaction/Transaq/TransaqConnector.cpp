...
pt::ptime ConvertTransacStringToPtime(const Source &source,
...
Connector::DataHandlers Connector::GetDataHandlers() const {
  ...
  const auto &insert = [this, &result](
      const char *tag,
      void (Connector::*hander)(const ptr::ptree &, const Milestones &,
                                const Milestones::TimePoint &)) {
    const auto &item = std::make_pair(
        tag, boost::bind(hander, const_cast<Connector *>(this), _1, _2, _3));
...
void Connector::Connect(const IniSectionRef &conf) {
  ...
  command->put("session_timeout", conf.ReadTypedKey<size_t>("session_timeout"));
  command->put("request_timeout", conf.ReadTypedKey<size_t>("request_timeout"));
  ...
  m_reconnectThread = boost::thread([this]() {
    try {
      RunReconnectionTaks();
    } catch (...) {
      AssertFailNoException();
...
void Connector::Connect(const ptr::ptree &command, Lock &lock) {
  ...
  m_log.Info("Connecting to TRANSAQ server at %1%:%2% as %3% with password...",
             command.get<std::string>("host"), command.get<std::string>("port"),
  ...
  const auto &result =
      SendCommand("connect", ptr::ptree(command), Milestones());
  if (!result.second.empty()) {
    throw ConnectException(result.second.c_str());
  }

  bool isTimeout = !m_connectCondition.timed_wait(
      lock, pt::seconds(command.get<long>("request_timeout") * 3));
...
void Connector::RunReconnectionTaks() {
  ...
  while (!m_isStopped) {
...

    {
      ...
      // http://moex.com/s96
      const pt::ptime sessionStartTime(m_context.GetCurrentTime().date(),
      ...
      if (sessionStartTime > m_context.GetCurrentTime()) {
        ...
        do {
          const auto waitTime =
              std::min(sessionStartTime - m_context.GetCurrentTime(),
                       pt::time_duration(pt::minutes(5)));
          m_reconnectCondition.timed_wait(lock, waitTime);
        } while (!m_isStopped && sessionStartTime > m_context.GetCurrentTime());
    ...
    if (now - *m_reconnectStartTime >= pt::minutes(7)) {
      m_log.Error(
          ...
          now - *m_reconnectStartTime, *m_reconnectStartTime, now);
    ...
    try {
      ...
      m_log.Error("Failed to reconnect to TRANSAQ server: \"%1%\"", ex.what());
      m_context.RaiseStateUpdate(Context::STATE_DISPATCHER_TASK_STOPPED_ERROR,
                                 "Failed to reconnect to TRANSAQ server");
      break;
    }
...
std::pair<boost::property_tree::ptree, std::string> Connector::SendCommand(
  ...
  try {
    ...
    result.second =
        ConvertUtf8ToAscii(result.first.get<std::string>("result.message"));
    if (result.second.empty()) {
      m_log.Error(
    ...
    m_log.Error("Failed to read TRANSAQ connector answer \"%1%\": \"%2%\".",
                &*resultData, ex.what());
    throw Exception("Failed to read TRANSAQ connector answer");
  }
}

void Connector::OnNewDataMessage(const ptr::ptree &message,
                                 const Milestones &delayMeasurement,
...
void Connector::OnErrorMessage(const ptr::ptree &message,
                               const Milestones &,
                               const Milestones::TimePoint &) {
  try {
    m_log.Error("TRANSAQ Connector error: \"%1%\".",
...
void Connector::OnMarketsInfoMessage(const ptr::ptree &message,
                                     const Milestones &,
                                     const Milestones::TimePoint &) {
  ...
  try {
    for (const auto node : message) {
      ...
      str % ConvertUtf8ToAscii(market.get_value<std::string>()) %
...
void Connector::OnBoardsInfoMessage(const ptr::ptree &message,
                                    const Milestones &,
                                    const Milestones::TimePoint &) {
  std::vector<std::string> list;
  try {
    for (const auto node : message) {
      ...
      str % ConvertUtf8ToAscii(board.get<std::string>("name")) %
...
void Connector::OnClientsInfoMessage(const ptr::ptree &message,
                                     const Milestones &,
                                     const Milestones::TimePoint &) {
  try {
    ...
    if (message.count("type")) {
      attributes.emplace_back("type: " + message.get<std::string>("type"));
    }
    if (message.count("currency")) {
      attributes.emplace_back("currency: " +
                              message.get<std::string>("currency"));
    }
    if (message.count("market")) {
      attributes.emplace_back("market: " + message.get<std::string>("market"));
    }
    if (message.count("union")) {
      attributes.emplace_back("union: " + message.get<std::string>("union"));
    }
    if (message.count("forts_acc")) {
      attributes.emplace_back("FORTS account: " +
                              message.get<std::string>("forts_acc"));
    }
    m_log.Info("Client account \"%1%\": %2%.",
...
void Connector::OnServerStatusMessage(const ptr::ptree &message,
                                      const Milestones &,
                                      const Milestones::TimePoint &) {
  ...
  try {
    ...
    const auto &connectStatus = message.get<std::string>("<xmlattr>.connected");
    const bool isError = connectStatus != "true" && connectStatus != "false";
    ...
    const bool isConnected = !isError && !isRecovery && connectStatus == "true";
    ...
    if (isConnected) {
      if (m_numberOfReconnectes++) {
        m_log.Warn("Connected to TRANSAQ server%1% with timezone \"%2%\".",
                   idFormat, message.get<std::string>("<xmlattr>.server_tz"));
      } else {
        m_log.Info("Connected to TRANSAQ server%1% with timezone \"%2%\".",
                   idFormat, message.get<std::string>("<xmlattr>.server_tz"));
      }
      ...
      m_log.Error("Failed to connect to TRANSAQ server%1%: \"%2%\".", idFormat,
                  ConvertUtf8ToAscii(message.get_value<std::string>()));
...
TradingConnector::TradingConnector(const Context &context,
                                   ModuleEventsLog &log,
  ...
  GetLog().Info(
      ...
      conf.IsKeyExist("union") ? m_defaultBuyOrder.get<std::string>("union")
                               : std::string());
}
...
TradingConnector::DataHandlers TradingConnector::GetDataHandlers() const {
  ...
  const auto &insert = [this, &result](
      const char *tag,
      void (TradingConnector::*hander)(const ptr::ptree &, const Milestones &,
                                       const Milestones::TimePoint &)) {
    const auto &item = std::make_pair(
        tag,
        boost::bind(hander, const_cast<TradingConnector *>(this), _1, _2, _3));
    Verify(result.emplace(std::move(item)).second);
  };
...
OrderId TradingConnector::SendSellOrder(const std::string &board,
                                        const std::string &symbol,
                                        double price,
                                        uint8_t pricePrecision,
                                        const Qty &qty,
                                        const Milestones &delayMeasurement) {
  return SendOrder(m_defaultSellOrder, board, symbol, price, pricePrecision,
                   qty, delayMeasurement);
}

OrderId TradingConnector::SendBuyOrder(const std::string &board,
                                       const std::string &symbol,
                                       double price,
                                       uint8_t pricePrecision,
                                       const Qty &qty,
                                       const Milestones &delayMeasurement) {
  return SendOrder(m_defaultBuyOrder, board, symbol, price, pricePrecision, qty,
                   delayMeasurement);
}

OrderId TradingConnector::SendOrder(const ptr::ptree &templateOrder,
  ...
  const auto &result =
      SendCommand("neworder", std::move(order), delayMeasurement);
  if (!result.second.empty()) {
...
bool TradingConnector::SendCancelOrder(const OrderId &orderId,
                                       const Milestones &delayMeasurement) {
  ptr::ptree message;
  message.put("transactionid", orderId);

  const auto &result =
      SendCommand("cancelorder", std::move(message), delayMeasurement);
  if (!result.second.empty()) {
    if (result.second.size() == 31 &&
...
void TradingConnector::OnOrdersMessage(const ptr::ptree &message,
                                       const Milestones &,
  ...
  try {
    for (const auto &subMessage : message) {
      ...
      if (status == numberOfOrderStatuses) {
        ...
        const auto &withdrawTime = order.get<std::string>("withdrawtime");
        if (withdrawTime.empty()) {
          continue;
        }
        AssertNe(ConvertTransacStringToPtime(withdrawTime, GetTimeZoneDiff()),
                 pt::not_a_date_time);

      } else {
        resultMessage = ConvertUtf8ToAscii(order.get<std::string>("result"));
      ...
      OnOrderUpdate(order.get<OrderId>("<xmlattr>.transactionid"),
                    order.get<TradingSystemOrderId>("orderno"),
                    std::move(status), order.get<double>("balance"),
                    std::move(resultMessage), replyTime);
    }
...
void TradingConnector::OnTradeMessage(const ptr::ptree &message,
                                      const Milestones &,
                                      const Milestones::TimePoint &) {
  try {
    for (const auto &subMessage : message) {
      ...
      OnTrade(trade.get<std::string>("tradeno"),
              trade.get<TradingSystemOrderId>("orderno"),
              trade.get<double>("price"), trade.get<double>("quantity"));
    }
  } catch (const std::exception &ex) {
...
MarketDataSourceConnector::MarketDataSourceConnector(const Context &context,
                                                     ModuleEventsLog &log)
    : Base(context, log) {
...
MarketDataSourceConnector::GetDataHandlers() const {
  DataHandlers result = Base::GetDataHandlers();

  const auto &insert = [this, &result](
      const char *tag, void (MarketDataSourceConnector::*hander)(
                           const ptr::ptree &, const Milestones &,
                           const Milestones::TimePoint &)) {
    const auto &item = std::make_pair(
        tag, boost::bind(hander, const_cast<MarketDataSourceConnector *>(this),
                         _1, _2, _3));
    Verify(result.emplace(std::move(item)).second);
  };
...
void MarketDataSourceConnector::SendMarketDataSubscriptionRequest(
  ...
  const auto &result =
      SendCommand("subscribe", std::move(command), Milestones());
  if (!result.second.empty()) {
    throw Exception(result.second.c_str());
  }
}
...
void MarketDataSourceConnector::OnTicksMessage(
  ...
  try {
    for (const auto &node : message) {
      ...
      OnNewTick(ConvertTransacStringToPtime(tick.get<std::string>("time"),
                                            GetTimeZoneDiff()),
                tick.get<std::string>("board"),
                tick.get<std::string>("seccode"), tick.get<double>("price"),
                tick.get<double>("quantity"), delayMeasurement);
    }
  } catch (const ptr::ptree_error &ex) {
...
void MarketDataSourceConnector::OnQuotationsMessage(
  ...
  try {
    for (const auto &node : message) {
      ...
      OnLevel1Update(update.get<std::string>("board"),
                     update.get<std::string>("seccode"), std::move(bidPrice),
                     std::move(bidQty), std::move(askPrice), std::move(askQty),
                     delayMeasurement);
    }
  } catch (const ptr::ptree_error &ex) {
    GetLog().Error("Failed to read Level 1 update message: \"%1%\".",
                   ex.what());
