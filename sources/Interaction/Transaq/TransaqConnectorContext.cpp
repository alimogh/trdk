...
TransaqConnectorContext::TransaqConnectorContext(const Context &context,
                                                 ModuleEventsLog &log) try
    : m_context(context),
      m_log(log),
      m_dll(sizeof(intptr_t) == 8 ? "txmlconnector64.dll"
                                  : "txmlconnector.dll"),
  ...
  const auto logPath = m_context.GetSettings().GetLogsInstanceDir() / "Transaq";
  m_log.Info("Logging TRANSAQ Connector (%1%) into %2%...", m_dll.GetFile(),
             logPath);
  fs::create_directories(logPath);

  const bool setCallbackResult = m_dll.GetFunction<ApiSetCallback>(
      "SetCallbackEx")(&TransaqConnectorContext::RaiseNewDataEvent, this);
  ...
  const ResultPtr initResult(
      m_dll.GetFunction<ApiInitialize>("Initialize")(logPath.string().c_str(),
                                                     logLevel),
      boost::bind(&TransaqConnectorContext::FreeMemory, this, _1));
  if (initResult) {
    m_log.Error("Failed to init TRANSAQ Connector: \"%1%\".", initResult.get());
    throw Exception("Failed to init TRANSAQ Connector");
  }
...
TransaqConnectorContext::~TransaqConnectorContext() {
  try {
    ...
    if (unInitializeResult) {
      m_log.Error("Failed to stop TRANSAQ Connector: \"%1%\".",
...
TransaqConnectorContext::ResultPtr TransaqConnectorContext::SendCommand(
    const char *argument, const Milestones &delayMeasurement) {
  delayMeasurement.Measure(TSM_ORDER_SEND);
  ResultPtr result(m_sendCommand(argument),
                   boost::bind(&TransaqConnectorContext::FreeMemory, this, _1));
...
bool TransaqConnectorContext::RaiseNewDataEvent(
    const char *data, TransaqConnectorContext *context) noexcept {
  try {
...
bool TransaqConnectorContext::OnNewData(const char *data) {
  ResultPtr dataHolder(
      data, boost::bind(&TransaqConnectorContext::FreeMemory, this, _1));
  ...
  Assert(dataHolder);
  if (!dataHolder) {
    m_log.Error("Failed to parse TRANSAQ connector message: data is empty.");
    return true;
  }
  ...
  ptr::ptree message;
  {
    ...
    try {
      ...
      m_log.Error("Failed to parse TRANSAQ connector message \"%1%\": \"%2%\".",
                  &*dataHolder, ex.what());
      return true;
    }
  }
