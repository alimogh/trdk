...
namespace {
	...
	public:
		...
		virtual ResultPtr SendCommand(
				const char *data,
				const lib::TimeMeasurement::Milestones &milestones)
				override {
			const auto &dummyDeleter = [](const char *) {
				//...//
			};
			return ResultPtr(
				SendCommandProxy(data, milestones),
				dummyDeleter);
		}
		...
		const char * CreateOrderCommandResult() {
			{
				...
				const_cast<std::string &>(m_orderCommandResultBuffer)
					= os.str();
			}
			TrdkAssert(
				boost::starts_with(
					m_orderCommandResultBuffer,
...
TEST_F(TransaqTradingSystemTest, CanceledAfter2Trades) {
...

	{
		...
		EXPECT_EQ("123134234", m_orderUpdates.back().tradingSystemOrderId);
		EXPECT_EQ(
			trdk::ORDER_STATUS_FILLED_PARTIALLY,
			m_orderUpdates.back().status);
		EXPECT_EQ(10, m_orderUpdates.back().remainingQty);
		ASSERT_TRUE(m_orderUpdates.back().tradeInfo);
		EXPECT_EQ(
			"sfgaer1",
			m_orderUpdates.back().tradeInfo->id);
		EXPECT_EQ(
			1,
			m_orderUpdates.back().tradeInfo->qty);
		EXPECT_EQ(
			66500,
			m_orderUpdates.back().tradeInfo->price);
	}
