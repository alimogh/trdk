...
Transaq::TradingSystem::TradingSystem(const TradingMode &mode,
                                      size_t index,
                                      Context &context,
  ...
  try {
    for (const auto &order : m_orders) {
      GetLog().Warn(
          ...
          order.id, order.tradingSystemOrderId, order.status, order.qty,
          order.remainingQty);
    }
  ...
  OrderId id;
  {
    ...
    try {
      id = TradingConnector::SendSellOrder(
          symbol.GetExchange(), symbol.GetSymbol(),
          security.DescalePrice(price), security.GetPricePrecision(), qty,
          delayMeasurement);
    } catch (const Exception &ex) {
      GetLog().Error("Failed to send order to sell: \"%1%\".", ex.what());
      throw SendingError("Failed to send order to sell");
    }
    RegisterOrder(security, id, qty, std::move(callback),
...
void Transaq::TradingSystem::SendCancelOrder(const OrderId &orderId) {
  ...
  } isExistent = IS_EXISTENT_YES;
  {
    ...
    if (order == index.cend() || order->remainingQty == 0 ||
      ...
      isExistent =
          ++m_unknownRequests[orderId] > 100 || m_unknownRequests.size() > 100
              ? IS_EXISTENT_TOO_MANY
              : IS_EXISTENT_NO;
    }
  }
  ...
  try {
    isOrderFound = TradingConnector::SendCancelOrder(orderId, delayMeasurement);
  } catch (const Exception &ex) {
...
void Transaq::TradingSystem::RegisterOrder(
  ...
  const Order order{
      &security, id,  std::move(callback), qty,
      qty,       qty, ORDER_STATUS_SENT,   std::move(delayMeasurement)};
...
void Transaq::TradingSystem::OnOrderUpdate(
  ...
  if (!tradingSystemMessage.empty()) {
    GetLog().Warn(
        ...
        id, tradingSystemOrderId, status, remainingQty, tradingSystemMessage);
  }
  ...
  boost::optional<Milestones> replyDelayMeasurement;
  {
      ...
      if (!m_tradingStarted) {
        GetTradingLog().Write(
            ...
            [&](TradingRecord &record) {
              record % id % tradingSystemOrderId % status % remainingQty %
        ...
        GetLog().Error(
            ...
            ", ts-id=%2%, status=%3%, remaining-qty=%4%"
            ", message=\"%5%\".",
            id, tradingSystemOrderId, status, remainingQty,
            tradingSystemMessage);
      }
      return;
    }
    ...
    if (order->orderBalance < remainingQty) {
      GetLog().Error(
          ...
          id, order->orderBalance, remainingQty);
    ...
    if (order->tradingSystemOrderId != tradingSystemOrderId) {
      ...
      order->replyDelayMeasurement.Add(TSM_ORDER_REPLY_RECEIVED, replyTime);
      index.modify(order, [&tradingSystemOrderId](Order &order) {
    ...
    switch (status) {
      case ORDER_STATUS_SUBMITTED:
        Assert(order->status == ORDER_STATUS_SENT ||
               order->status == ORDER_STATUS_SUBMITTED);
  ...
  Assert(callback);
  callback(id, boost::lexical_cast<std::string>(tradingSystemOrderId), status,
           remainingQty, nullptr);
  ...
  OrderStatus status;
  {
    ...
    if (it == index.cend()) {
      if (!m_tradingStarted) {
        GetTradingLog().Write(
            ...
            [&](TradingRecord &record) {
              record % id % tradingSystemOrderId % price % qty;
            });
      } else {
        GetLog().Error(
            "Unknown trade received from TRANSAQ server"
            ": id=%1%, ts-order-id=%2%, price=%3%, qty=%4%.",
            id, tradingSystemOrderId, price, qty);
      }
      return;
    }

    Assert(it->status == ORDER_STATUS_SUBMITTED ||
    ...
    switch (it->status) {
      ...
      case ORDER_STATUS_CANCELLED:
        ...
        break;
      default:
        status = it->remainingQty ? ORDER_STATUS_FILLED_PARTIALLY
                                  : ORDER_STATUS_FILLED;
        break;
    }
  }

  const TradeInfo trade{std::move(id), std::move(qty),
                        order.security->ScalePrice(price)};
  order.callback(order.id,
                 boost::lexical_cast<std::string>(tradingSystemOrderId), status,
                 order.remainingQty, &trade);
}
...
boost::shared_ptr<trdk::TradingSystem> CreateTradingSystem(
  ...
  return boost::make_shared<Transaq::TradingSystem>(
      mode, tradingSystemIndex, context, instanceName, conf);
}
