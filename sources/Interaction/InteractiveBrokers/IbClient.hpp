...
class Client : protected EWrapper {
 ...
 private:
  ...
  enum PingState { PING_STATE_IDLE, PING_STATE_REQ, PING_STATE_ACK };
  ...
  struct SecurityRequest {
    ...
    explicit SecurityRequest(Security &security, const TickerId &tickerId)
        : security(&security), tickerId(tickerId) {}

    const Lib::Symbol &GetSymbol() const { return security->GetSymbol(); }
  };
  typedef boost::multi_index_container<
      ...
      boost::multi_index::indexed_by<
          boost::multi_index::hashed_non_unique<
              ...
              boost::multi_index::member<SecurityRequest,
                                         Security *,
 ...
 public:
  Client(InteractiveBrokers::TradingSystem &,
 ...
 public:
  ...
  trdk::OrderId PlaceBuyOrder(const trdk::Security &,
                              ...
                              double price,
                              const OrderParams &);
  trdk::OrderId PlaceBuyOrderWithMarketPrice(const trdk::Security &,
                                             Qty qty,
                                             double stopPrice,
                                             const OrderParams &);
  trdk::OrderId PlaceBuyIocOrder(const trdk::Security &,
                                 Qty,
                                 double,
                                 const OrderParams &);

  trdk::OrderId PlaceSellOrder(const trdk::Security &,
                               Qty qty,
                               const OrderParams &);
  trdk::OrderId PlaceSellOrder(const trdk::Security &,
                               Qty quantity,
                               double price,
                               const OrderParams &);
  trdk::OrderId PlaceSellOrderWithMarketPrice(const trdk::Security &,
                                              Qty qty,
                                              double stopPrice,
                                              const OrderParams &);
  trdk::OrderId PlaceSellIocOrder(const trdk::Security &,
                                  Qty,
                                  double,
                                  const OrderParams &);
 ...
 private:
  ...
  boost::unordered_map<Security *, std::vector<HistoryUpdate>> m_historyUpdates;
