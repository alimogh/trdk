...
struct ComboLeg {
  ComboLeg()
      : conId(0), ratio(0), openClose(0), shortSaleSlot(0), exemptCode(-1) {}
  ...
  bool operator==(const ComboLeg& other) const {
    return (conId == other.conId && ratio == other.ratio &&
...
struct UnderComp {
  UnderComp() : conId(0), delta(0), price(0) {}
...
struct Contract {
  ...
  IBString multiplier;
  IBString exchange;
  IBString primaryExchange;  // pick an actual (ie non-aggregate) exchange that
                             // the contract trades on.  DO NOT SET TO SMART.
  IBString currency;
...
inline void Contract::CloneComboLegs(ComboLegListSPtr& dst,
                                     const ComboLegListSPtr& src) {
  if (!src.get()) return;
