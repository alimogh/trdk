...
struct Order {
  ...
  int ocaType;        // 1 = CANCEL_WITH_BLOCK, 2 = REDUCE_WITH_BLOCK, 3 =
                      // REDUCE_NON_BLOCK
  IBString orderRef;  // order reference
  bool transmit;      // if false, order will be created but not transmited
  long parentId;  // Parent order Id, to associate Auto STP or TRAIL orders with
                  // the original order.
  bool blockOrder;
  bool sweepToFill;
  int displaySize;
  int triggerMethod;  // 0=Default, 1=Double_Bid_Ask, 2=Last, 3=Double_Last,
  ...
  IBString rule80A;  // Individual = 'I', Agency = 'A', AgentOtherMember = 'W',
                     // IndividualPTIA = 'J', AgencyPTIA = 'U',
                     // AgentOtherMemberPTIA = 'M', IndividualPT = 'K', AgencyPT
                     // = 'Y', AgentOtherMemberPT = 'N'
  bool allOrNone;
  int minQty;
  ...
  int shortSaleSlot;   // 1 if you hold the shares, 2 if they will be delivered
                       // from elsewhere.  Only for Action="SSHORT
  ...
  // BOX exchange orders only
  int auctionStrategy;  // AUCTION_MATCH, AUCTION_IMPROVEMENT,
                        // AUCTION_TRANSPARENT
...
inline void Order::CloneOrderComboLegs(OrderComboLegListSPtr& dst,
                                       const OrderComboLegListSPtr& src) {
  if (!src.get()) return;
