...
///////////////////////////////////////////////////////////
// helper macroses
#define DECODE_FIELD(x) \
  if (!DecodeField(x, ptr, endPtr)) return 0;
#define DECODE_FIELD_MAX(x) \
  if (!DecodeFieldMax(x, ptr, endPtr)) return 0;
...
template <>
void EClientSocketBase::EncodeField<double>(std::ostream& os,
                                            double doubleValue) {
  char str[128];
...
const char* EClientSocketBase::FindFieldEnd(const char* ptr,
                                            const char* endPtr) {
  return (const char*)memchr(ptr, 0, endPtr - ptr);
}

bool EClientSocketBase::DecodeField(bool& boolValue,
                                    const char*& ptr,
                                    const char* endPtr) {
  int intValue;
  if (!DecodeField(intValue, ptr, endPtr)) return false;
  boolValue = (intValue > 0);
  return true;
}

bool EClientSocketBase::DecodeField(int& intValue,
                                    const char*& ptr,
                                    const char* endPtr) {
  if (!CheckOffset(ptr, endPtr)) return false;
...
bool EClientSocketBase::DecodeField(long& longValue,
                                    const char*& ptr,
                                    const char* endPtr) {
  int intValue;
  if (!DecodeField(intValue, ptr, endPtr)) return false;
...
bool EClientSocketBase::DecodeField(double& doubleValue,
                                    const char*& ptr,
                                    const char* endPtr) {
  if (!CheckOffset(ptr, endPtr)) return false;
  const char* fieldBeg = ptr;
...
bool EClientSocketBase::DecodeFieldMax(int& intValue,
                                       const char*& ptr,
                                       const char* endPtr) {
  IBString stringValue;
  if (!DecodeField(stringValue, ptr, endPtr)) return false;
  intValue = (IsEmpty(stringValue) ? UNSET_INTEGER : Atoi(stringValue));
  return true;
}

bool EClientSocketBase::DecodeFieldMax(long& longValue,
                                       const char*& ptr,
                                       const char* endPtr) {
  int intValue;
  if (!DecodeFieldMax(intValue, ptr, endPtr)) return false;
  longValue = intValue;
  return true;
}

bool EClientSocketBase::DecodeFieldMax(double& doubleValue,
                                       const char*& ptr,
                                       const char* endPtr) {
  IBString stringValue;
  if (!DecodeField(stringValue, ptr, endPtr)) return false;
...
int EClientSocketBase::serverVersion() { return m_serverVersion; }

IBString EClientSocketBase::TwsConnectionTime() { return m_TwsTime; }

void EClientSocketBase::reqMktData(TickerId tickerId,
  ...
  if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
    ...
    const int mktDataOptionsCount =
        mktDataOptions.get() ? mktDataOptions->size() : 0;
...
void EClientSocketBase::reqMktDepth(TickerId tickerId,
                                    const Contract& contract,
                                    int numRows,
  ...
  if (m_serverVersion < MIN_SERVER_VER_TRADING_CLASS) {
    if (!IsEmpty(contract.tradingClass) || (contract.conId > 0)) {
      m_pEWrapper->error(tickerId, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() +
                             "  It does not support conId and tradingClass "
                             "parameters in reqMktDepth.");
      return;
    }
  }
  ...
  if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
    ...
    const int mktDepthOptionsCount =
        mktDepthOptions.get() ? mktDepthOptions->size() : 0;
...
void EClientSocketBase::reqHistoricalData(
    TickerId tickerId,
    const Contract& contract,
...
void EClientSocketBase::reqRealTimeBars(
    TickerId tickerId,
    const Contract& contract,
    int barSize,
    const IBString& whatToShow,
  ...
  if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
    ...
    const int realTimeBarsOptionsCount =
        realTimeBarsOptions.get() ? realTimeBarsOptions->size() : 0;
...
void EClientSocketBase::reqScannerSubscription(
  ...
  if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
    ...
    const int scannerSubscriptionOptionsCount =
        scannerSubscriptionOptions.get() ? scannerSubscriptionOptions->size()
                                         : 0;
...
void EClientSocketBase::reqFundamentalData(TickerId reqId,
                                           const Contract& contract,
...
void EClientSocketBase::calculateImpliedVolatility(TickerId reqId,
                                                   const Contract& contract,
                                                   double optionPrice,
                                                   double underPrice) {
  // not connected?
  ...
  if (m_serverVersion < MIN_SERVER_VER_TRADING_CLASS) {
    if (!IsEmpty(contract.tradingClass)) {
      m_pEWrapper->error(reqId, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() +
                             "  It does not support tradingClass parameter in "
...
void EClientSocketBase::calculateOptionPrice(TickerId reqId,
                                             const Contract& contract,
                                             double volatility,
                                             double underPrice) {
  // not connected?
...
void EClientSocketBase::reqContractDetails(int reqId,
                                           const Contract& contract) {
  // not connected?
  if (!m_connected) {
    m_pEWrapper->error(NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void EClientSocketBase::placeOrder(OrderId id,
                                   const Contract& contract,
                                   const Order& order) {
  // not connected?
  if (!m_connected) {
  ...
  //				m_pEWrapper->error( id, UPDATE_TWS.code(), UPDATE_TWS.msg()
  //+
  //					"  It does not support SSHORT flag for combo
  //legs.");
  //				return;
  //			}
  //		}
  ...
  if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_CONID) {
    if (order.deltaNeutralConId > 0 ||
      ...
      m_pEWrapper->error(id, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() +
                             "  It does not support deltaNeutral parameters: "
                             "ConId, SettlingFirm, ClearingAccount, "
                             "ClearingIntent.");
      return;
    }
  }
  ...
  if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE) {
    if (!IsEmpty(order.deltaNeutralOpenClose) || order.deltaNeutralShortSale ||
        order.deltaNeutralShortSaleSlot > 0 ||
        !IsEmpty(order.deltaNeutralDesignatedLocation)) {
      m_pEWrapper->error(id, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() +
                             "  It does not support deltaNeutral parameters: "
                             "OpenClose, ShortSale, ShortSaleSlot, "
                             "DesignatedLocation.");
      return;
    }
  }
  ...
  if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3) {
    if (order.scalePriceIncrement > 0 &&
      ...
      if (order.scalePriceAdjustValue != UNSET_DOUBLE ||
          ...
          order.scaleProfitOffset != UNSET_DOUBLE || order.scaleAutoReset ||
          order.scaleInitPosition != UNSET_INTEGER ||
          order.scaleInitFillQty != UNSET_INTEGER || order.scaleRandomPercent) {
        m_pEWrapper->error(id, UPDATE_TWS.code(),
                           UPDATE_TWS.msg() +
                               "  It does not support Scale order parameters: "
                               "PriceAdjustValue, PriceAdjustInterval, " +
                               "ProfitOffset, AutoReset, InitPosition, "
                               "InitFillQty and RandomPercent");
        return;
      }
    }
  }
  ...
  if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE &&
      Compare(contract.secType, "BAG") == 0) {
    const Order::OrderComboLegList* const orderComboLegs =
        order.orderComboLegs.get();
  ...
  if (m_serverVersion < MIN_SERVER_VER_SCALE_TABLE) {
    if (!IsEmpty(order.scaleTable) || !IsEmpty(order.activeStartTime) ||
        !IsEmpty(order.activeStopTime)) {
      m_pEWrapper->error(id, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() +
                             "  It does not support scaleTable, "
                             "activeStartTime and activeStopTime parameters");
      return;
    }
  }
  ...
  if (m_serverVersion >= MIN_SERVER_VER_SMART_COMBO_ROUTING_PARAMS &&
      Compare(contract.secType, "BAG") == 0) {
    const TagValueList* const smartComboRoutingParams =
        order.smartComboRoutingParams.get();
    const int smartComboRoutingParamsCount =
        smartComboRoutingParams ? smartComboRoutingParams->size() : 0;
  ...
  // E.g.
  //		To allocate 20 shares of a 100 share order to account 'U101' and
  //the
  ...
  ENCODE_FIELD(order.auctionStrategy);  // AUCTION_MATCH, AUCTION_IMPROVEMENT,
                                        // AUCTION_TRANSPARENT
  ENCODE_FIELD_MAX(order.startingPrice);
  ENCODE_FIELD_MAX(order.stockRefPrice);
  ENCODE_FIELD_MAX(order.delta);
  // Volatility orders had specific watermark price attribs in server version 26
  // double lower = (m_serverVersion == 26 && isVolOrder) ? DBL_MAX :
  // order.stockRangeLower;
  // double upper = (m_serverVersion == 26 && isVolOrder) ? DBL_MAX :
  ...
  if (m_serverVersion >= MIN_SERVER_VER_LINKING) {
    ...
    const int orderMiscOptionsCount =
        orderMiscOptions ? orderMiscOptions->size() : 0;
...
void EClientSocketBase::reqAccountUpdates(bool subscribe,
...
void EClientSocketBase::reqExecutions(int reqId,
...
void EClientSocketBase::replaceFA(faDataType pFaDataType,
                                  const IBString& cxml) {
  // not connected?
  if (!m_connected) {
    m_pEWrapper->error(NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
...
void EClientSocketBase::exerciseOptions(TickerId tickerId,
                                        const Contract& contract,
                                        int exerciseAction,
  ...
  if (m_serverVersion < MIN_SERVER_VER_TRADING_CLASS) {
    if (!IsEmpty(contract.tradingClass) || (contract.conId > 0)) {
      m_pEWrapper->error(tickerId, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() +
                             "  It does not support conId, multiplier and "
                             "tradingClass parameters in exerciseOptions.");
      return;
    }
  }
...
void EClientSocketBase::reqAccountSummary(int reqId,
...
void EClientSocketBase::verifyRequest(const IBString& apiName,
  ...
  if (!m_extraAuth) {
    m_pEWrapper->error(NO_VALID_ID, UPDATE_TWS.code(),
                       UPDATE_TWS.msg() +
                           "  Intent to authenticate needs to be expressed "
                           "during initial connect request.");
    return;
  }
...
void EClientSocketBase::updateDisplayGroup(int reqId,
...
bool EClientSocketBase::checkMessages() {
  ...
  try {
    while ((m_connected ? processMsg(ptr, endPtr)
      ...
      if ((ptr - beginPtr) >= (int)m_inBuffer.size()) break;
    }
  } catch (...) {
...
int EClientSocketBase::processConnectAck(const char*& beginPtr,
                                         const char* endPtr) {
  ...
  try {
    ...
    switch (msgId) {
      case TICK_PRICE: {
        ...
        m_pEWrapper->tickPrice(tickerId, (TickType)tickTypeInt, price,
                               canAutoExecute);
      ...
      case TICK_OPTION_COMPUTATION: {
        ...
        if (version >= 6 ||
            tickTypeInt == MODEL_OPTION) {  // introduced in version == 5
        ...
        if (version >= 6) {
          ...
          if (gamma > 1 ||
              gamma < -1) {  // -2 is the "not yet computed" indicator
            gamma = DBL_MAX;
          }
          if (vega > 1 ||
              vega < -1) {  // -2 is the "not yet computed" indicator
            vega = DBL_MAX;
          }
          if (theta > 1 ||
              theta < -1) {  // -2 is the "not yet computed" indicator
            theta = DBL_MAX;
          }
        ...
        m_pEWrapper->tickOptionComputation(
            tickerId, (TickType)tickTypeInt, impliedVol, delta, optPrice,
      ...
      case TICK_EFP: {
        ...
        m_pEWrapper->tickEFP(tickerId, (TickType)tickTypeInt, basisPoints,
      ...
      case OPEN_ORDER: {
        ...
        if (version >= 29) {
          ...
          if (orderComboLegsCount > 0) {
            Order::OrderComboLegListSPtr orderComboLegs(
        ...
        m_pEWrapper->openOrder((OrderId)order.orderId, contract, order,
                               orderState);
        break;
      }
      ...
      case PORTFOLIO_VALUE: {
        ...
        m_pEWrapper->updatePortfolio(contract, position, marketPrice,
      ...
      case MARKET_DEPTH_L2: {
        ...
        m_pEWrapper->updateMktDepthL2(id, position, marketMaker, operation,
                                      side, price, size);
      ...
      case NEWS_BULLETINS: {
        ...
        m_pEWrapper->updateNewsBulletin(msgId, msgType, newsMessage,
                                        originatingExch);
        break;
      }
      ...
      case HISTORICAL_DATA: {
        ...
        for (int ctr = 0; ctr < itemCount; ++ctr) {
          ...
          m_pEWrapper->historicalData(reqId, bar.date, bar.open, bar.high,
        ...
        IBString finishedStr =
            IBString("finished-") + startDateStr + "-" + endDateStr;
        m_pEWrapper->historicalData(reqId, finishedStr, -1, -1, -1, -1, -1, -1,
                                    -1, 0);
        break;
      }
      ...
      case SCANNER_DATA: {
        ...
        for (int ctr = 0; ctr < numberOfElements; ++ctr) {
          ...
          m_pEWrapper->scannerData(tickerId, data.rank, data.contract,
                                   data.distance, data.benchmark,
                                   data.projection, data.legsStr);
        }
      ...
      case REAL_TIME_BARS: {
        ...
        m_pEWrapper->realtimeBar(reqId, time, open, high, low, close, volume,
                                 average, count);
...
bool EClientSocketBase::isConnected() const { return m_connected; }

EWrapper* EClientSocketBase::getWrapper() const { return m_pEWrapper; }

void EClientSocketBase::setClientId(int clientId) { m_clientId = clientId; }
...
bool EClientSocketBase::isInBufferEmpty() const { return m_inBuffer.empty(); }

bool EClientSocketBase::isOutBufferEmpty() const { return m_outBuffer.empty(); }
