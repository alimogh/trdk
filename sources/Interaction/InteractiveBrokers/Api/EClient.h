...
class EClient {
 public:
  ...
  virtual void reqMktData(TickerId id,
                          const Contract &contract,
                          const IBString &genericTicks,
                          bool snapshot,
                          const TagValueListSPtr &mktDataOptions) = 0;
  virtual void cancelMktData(TickerId id) = 0;
  virtual void placeOrder(OrderId id,
                          const Contract &contract,
                          const Order &order) = 0;
  ...
  virtual void reqMktDepth(TickerId id,
                           const Contract &contract,
                           int numRows,
                           const TagValueListSPtr &mktDepthOptions) = 0;
  ...
  virtual void reqHistoricalData(TickerId id,
                                 const Contract &contract,
                                 ...
                                 int useRTH,
                                 int formatDate,
                                 const TagValueListSPtr &chartOptions) = 0;
  virtual void exerciseOptions(TickerId id,
                               const Contract &contract,
                               int exerciseAction,
  ...
  virtual void reqRealTimeBars(TickerId id,
                               const Contract &contract,
                               int barSize,
                               const IBString &whatToShow,
                               bool useRTH,
                               const TagValueListSPtr &realTimeBarsOptions) = 0;
  ...
  virtual void reqScannerSubscription(
      int tickerId,
      const ScannerSubscription &subscription,
      const TagValueListSPtr &scannerSubscriptionOptions) = 0;
  virtual void reqCurrentTime() = 0;
  virtual void reqFundamentalData(TickerId reqId,
                                  const Contract &,
                                  const IBString &reportType) = 0;
  virtual void cancelFundamentalData(TickerId reqId) = 0;
  virtual void calculateImpliedVolatility(TickerId reqId,
                                          const Contract &contract,
                                          double optionPrice,
                                          double underPrice) = 0;
  virtual void calculateOptionPrice(TickerId reqId,
                                    const Contract &contract,
                                    double volatility,
                                    double underPrice) = 0;
  ...
  virtual void reqAccountSummary(int reqId,
                                 const IBString &groupName,
                                 const IBString &tags) = 0;
  virtual void cancelAccountSummary(int reqId) = 0;
  virtual void verifyRequest(const IBString &apiName,
                             const IBString &apiVersion) = 0;
