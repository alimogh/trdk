    ...
    try {
      for (;;) {
        const auto eventCode = WaitForMultipleObjects(
            _countof(events), &events[0], FALSE, INFINITE);
        switch (eventCode) {
          default:
...
FileSystemChangeNotificator::FileSystemChangeNotificator(
    const fs::path &path, const EventSlot &eventSlot)
...
void FileSystemChangeNotificator::Start() {
  ...
  m_pimpl->watchHandle =
      FindFirstChangeNotification(m_pimpl->path.branch_path().string().c_str(),
...
class FileSystemChangeNotificator::Implementation : private boost::noncopyable {
 public:
  ...
  explicit Implementation(
      const fs::path &path,
      const FileSystemChangeNotificator::EventSlot &eventSlot)
      : m_path(path), m_eventSlot(eventSlot), m_stopFlag(true) {}
  ...
  void Task() {
    try {
      ...
      while (!m_stopFlag) {
        const auto newTime =
            fs::exists(m_path) ? fs::last_write_time(m_path) : 0;
        if (newTime != lastTime) {
          if (lastTime != 0) {
            m_eventSlot();
          }
          ...
          m_condition.wait_for(lock, boost::chrono::seconds(1));
        }
      }
    } catch (...) {
      AssertFail("File System Change Notificator error.");
...
FileSystemChangeNotificator::FileSystemChangeNotificator(const fs::path &path,
...
void FileSystemChangeNotificator::Stop() { m_pimpl->Stop(); }
