...
class VersionInfoV1 {
...
#ifdef DEV_VER
    {
      const std::string moduleList[] = TRDK_GET_MODULE_FILE_NAME_LIST();
      bool isFound = false;
 ...
 public:
  bool operator==(const VersionInfoV1 &lhs) const {
    return m_release == lhs.m_release && m_build == lhs.m_build &&
           m_status == lhs.m_status &&
           m_concurrencyProfile == lhs.m_concurrencyProfile &&
           m_branch == lhs.m_branch && m_vendor == lhs.m_vendor &&
           m_buildConfiguration == lhs.m_buildConfiguration &&
           m_moduleFileName == lhs.m_moduleFileName;
  }
  bool operator!=(const VersionInfoV1 &lhs) const { return !operator==(lhs); }
  ...
  friend std::basic_ostream<StreamElem, StreamTraits> &operator<<(
    ...
    os << lhs.m_release << '.' << lhs.m_build << '.' << lhs.m_status
       << "::" << lhs.m_branch << "::" << lhs.m_vendor
       << "::" << lhs.m_concurrencyProfile << "::" << lhs.m_buildConfiguration
