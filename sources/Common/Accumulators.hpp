...
struct MovingAverageExponential : public accumulator_base {
  ...
  void operator()(const Args &args) {
    ...
    m_sum = (args[sample] * m_smoothingConstant) +
            (m_sum * (1 - m_smoothingConstant));
  }

  const result_type &result(const dont_care &) const { return m_sum; }
...
struct MovingAverageSmoothed : public accumulator_base {
  ...
  explicit MovingAverageSmoothed(const Args &args)
      : m_windowSize(args[rolling_window_size]), m_val(0) {}
  ...
  const result_type &result(const dont_care &) const { return m_val; }
...
typedef boost::accumulators::accumulator_set<
    double,
    boost::accumulators::stats<boost::accumulators::tag::MovingAverageSmoothed>>
    Smoothed;
}
}
}
