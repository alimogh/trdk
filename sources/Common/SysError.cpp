...
SysError::SysError(int errorNo) throw() : m_errorNo(errorNo) {}
...
bool SysError::CheckError() const {
  ...
  const auto formatResult = ::FormatMessageW(
      ...
      NULL, m_errorNo, 0, (LPWSTR)&buffer, 0, NULL);
...
String GetStringFromSys(int errorNo, SysGetter &sysGetter) {
  LPVOID buffer;
  auto size =
      sysGetter(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                    FORMAT_MESSAGE_IGNORE_INSERTS,
                NULL, errorNo, 0, (String::value_type *)&buffer, 0, NULL);
  boost::shared_ptr<VOID> bufferPtr(buffer, &::LocalFree);
...
#else   // BOOST_WINDOWS
  return boost::system::system_error(m_errorNo,
                                     boost::system::get_system_category())
      .what();
#endif  // BOOST_WINDOWS
}

int SysError::GetErrorNo() const { return m_errorNo; }
...
std::wostream &trdk::Lib::operator<<(std::wostream &os, const SysError &error) {
