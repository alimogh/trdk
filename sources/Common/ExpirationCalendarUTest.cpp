...
TEST_F(ExpirationCalendarTest, FindByDate) {
  ...
  {
    auto it =
        calendar.Find(lib::Symbol("XXX1/USD::FUT"), gr::date(2016, gr::Apr, 6));
    ASSERT_TRUE(it);
  ...
  {
    auto it = calendar.Find(lib::Symbol("XXX1/USD::FUT"),
  ...
  // {"XXX2", 20151110},
  {
    auto it = calendar.Find(lib::Symbol("XXX2/USD::FUT"),
  ...
  {
    EXPECT_FALSE(calendar.Find(lib::Symbol("XXX2/USD::FUT"),
  ...
  {
    auto it =
        calendar.Find(lib::Symbol("XXX2/USD::FUT"), gr::date(2016, gr::Feb, 1));
    ASSERT_TRUE(it);
...
TEST_F(ExpirationCalendarTest, WrongInstrumnetType) {
  ...
  for (int i = 0; i < lib::numberOfSecurityTypes; ++i) {
    if (i == lib::SECURITY_TYPE_FUTURES
    ...
    const lib::Symbol symbol("XXX1/USD::" + lib::ConvertToString(securityType));
    boost::format error(
...
TEST_F(ExpirationCalendarTest, Iterator) {
  ...
  // {"XXX2", 20151110},
  auto it1 =
      calendar.Find(lib::Symbol("XXX2/USD::FUT"), gr::date(2015, gr::Sep, 26));
  ASSERT_TRUE(it1);
