...
namespace trdk { namespace Tests {
	...
	TEST(BarServiceTest, Level1TickByTick) {
		...
		const size_t numberOfSets = 35;
		const Source &source = GetSource(numberOfSets);

		std::vector<size_t> tradesPos;
		for (size_t i = 0, tradesCount = 0; i < source.size(); ++i) {
			...
			if (service.OnLevel1Tick(security, tick.time, tick.value)) {
				...
				tradesCount = 0;
				tradesPos.emplace_back(i);
//! @todo add last actual bar test
			} else {
				ASSERT_GT(10, tradesCount) << "i = " << i << ";";
			}
			if (tick.value.GetType() == LEVEL1_TICK_LAST_QTY) {
				++tradesCount;
			}
		}
		...
		EXPECT_EQ(29 + 3, service.GetLastBar().openBidPrice);
		// 28 - bid is opened bar, so ask-open will be "restored" from prev-bar:
		EXPECT_EQ(28 + 5, service.GetLastBar().openAskPrice);

		EXPECT_EQ(34 + 3, service.GetLastBar().closeBidPrice);
		EXPECT_EQ(34 + 5, service.GetLastBar().closeAskPrice);

		EXPECT_EQ(29 + 3, service.GetLastBar().minBidPrice);
		EXPECT_EQ(34 + 5, service.GetLastBar().maxAskPrice);
		
		EXPECT_EQ(29 + 1, service.GetLastBar().openTradePrice); // prev close
		EXPECT_EQ(34 + 1, service.GetLastBar().closeTradePrice);
 		EXPECT_EQ(34 + 1, service.GetLastBar().highTradePrice);
 		EXPECT_EQ(29 + 1, service.GetLastBar().lowTradePrice);
		EXPECT_EQ(
