...
#endif
  maSourcesMap.emplace(maSource, std::make_pair<std::string, MaSourceInfo>(
                                     keyVal, MaSourceToType<maSource>()));
}
...
class AccumVisitor : public boost::static_visitor<void> {
 public:
  explicit AccumVisitor(double frameValue) : m_frameValue(frameValue) {}
...
struct GetValueVisitor : public boost::static_visitor<double> {
  double operator()(ma::Simple &acc) const { return accs::rolling_mean(acc); }
  double operator()(ma::Exponential &acc) const { return accs::ema(acc); }
  double operator()(ma::Smoothed &acc) const { return accs::smma(acc); }
};
}
...
MovingAverageService::HasNotHistory::HasNotHistory(const char *what) throw()
    : Error(what) {}
...
class MovingAverageService::Implementation : private boost::noncopyable {
 ...
 public:
  explicit Implementation(MovingAverageService &service,
                          const IniSectionRef &configuration)
      : m_service(service),
        m_sourceInfo(MaSourceToType<MA_SOURCE_CLOSE_PRICE>()),
        m_lastValueNo(0) {
    m_period =
        configuration.ReadTypedKey<uintmax_t>(Configuration::Keys::period);
    if (m_period <= 1) {
    ...
    MaType type = numberOfMaTypes;
    {
      ...
      switch (type) {
        case MA_TYPE_SIMPLE: {
          const ma::Simple acc(accs::tag::rolling_window::window_size =
                                   m_period);
          m_acc.reset(new Acc(acc));
        } break;
        case MA_TYPE_EXPONENTIAL: {
          const ma::Exponential acc(accs::tag::rolling_window::window_size =
                                        m_period);
          m_acc.reset(new Acc(acc));
        } break;
        case MA_TYPE_SMOOTHED: {
          const ma::Smoothed acc(accs::tag::rolling_window::window_size =
                                     m_period);
          m_acc.reset(new Acc(acc));
        } break;
        case MA_TYPE_HULL:
    ...
    m_service.GetLog().Info(
        ...
        Configuration::Keys::type, types[type], Configuration::Keys::period,
        m_period, Configuration::Keys::source,
  ...
  void LogPoint(const Point &point) {
    ...
    m_pointsLog << point.time.date() << ','
                << ExcelTextField(point.time.time_of_day()) << ','
                << point.source << ',' << point.value << std::endl;
  }
  ...
  bool OnNewValue(const pt::ptime &valueTime, double newValue) {
    ...
    const Point newPoint = {valueTime, newValue,
...
void MovingAverageService::DropLastPointCopy(
  ...
  GetContext().InvokeDropCopy([this, &sourceId](DropCopy &dropCopy) {
    ...
    dropCopy.CopyAbstractData(sourceId, m_pimpl->m_lastValueNo - 1,
...
MovingAverageService::GetHistoryPointByReversedIndex(size_t index) const {
  m_pimpl->CheckHistoryIndex(index);
...
TRDK_SERVICES_API boost::shared_ptr<trdk::Service> CreateMovingAverageService(
    ...
    const std::string &instanceName,
    const IniSectionRef &configuration) {
  return boost::make_shared<MovingAverageService>(context, instanceName,
                                                  configuration);
}
