...
Rsi::Error::Error(const char *what) noexcept : Exception(what) {}
...
void Rsi::DropLastPointCopy(
  ...
  GetContext().InvokeDropCopy([this, &sourceId](DropCopy &dropCopy) {
    ...
    dropCopy.CopyAbstractData(sourceId, m_pimpl->m_lastValueNo - 1, point.time,
                              point.value);
  });
}

bool Rsi::OnServiceDataUpdate(const Service &service,
  ...
  return m_pimpl->OnNewValue(bar.endTime, bar.closeTradePrice);
}

////////////////////////////////////////////////////////////////////////////////

TRDK_SERVICES_API boost::shared_ptr<trdk::Service> CreateRsiIndicatorService(
    Context &context,
    const std::string &instanceName,
