...
class ContinuousContractBarService::Implementation
 ...
 public:
  ...
  explicit Implementation(ContinuousContractBarService &self,
                          Context &context,
  ...
  void RestartLog() {
    ...
    m_log.open(path.string(), std::ios::out | std::ios::ate | std::ios::app);
    if (!m_log.is_open()) {
      m_self.GetLog().Error(
          "Failed to open continuous contract bars log file %1%", path);
      return;
    }
    if (isNew) {
      m_log << "Date" << ',' << "Time" << ',' << "Contract" << ','
            << "Expiration" << ',' << "Open" << ',' << "High" << ',' << "Low"
            << ',' << "Close" << std::endl;
    }
    m_log << std::setfill('0');

    m_self.GetLog().Info("Logging continuous contract bars into %1%.", path);
  ...
  void Log(const Bar &bar, const ContractExpiration &expiration) const {
  ...
  void Build() {
    ...
    double ratio = 0;
    m_source.ForEachReversed([&](const BarService::Bar &source) -> bool {
      ...
      if (!isActual) {
        ...
        if (!closeNext) {
          ...
          ratio = double(closeNext->closeTradePrice) / source.closeTradePrice;
          closeNext = nullptr;
        }
      }
    ...
    m_self.GetLog().Debug(
        ...
        m_self.GetSecurity(), m_bars.size(), numberOfBarsForCurrentContracts,
  ...
  void OnNewBar() {
    ...
    m_meta.emplace_back(BarMeta{m_currentHistoryEnd.is_not_a_date_time(),
...
ContinuousContractBarService::ContinuousContractBarService(
    ...
    : Base(context,
      ...
      m_pimpl(std::make_unique<Implementation>(
          *this, context, instanceName, conf)) {}
...
bool ContinuousContractBarService::IsEmpty() const { return GetSize() == 0; }
...
void ContinuousContractBarService::OnSecurityStart(const Security &security,
  ...
  if (security.GetSymbol().GetSecurityType() != SECURITY_TYPE_FUTURES) {
    ...
    message % security.GetSymbol().GetSecurityType() % security;
...
void ContinuousContractBarService::OnSecurityContractSwitched(
  ...
  if (m_pimpl->m_currentHistoryEnd.is_not_a_date_time()) {
    const pt::ptime nextHour(
        time.date(), pt::time_duration(time.time_of_day().hours(), 0, 0));
    ...
    GetLog().Error(
        ...
        GetSecurity(), m_pimpl->m_currentHistoryEnd);
...
bool ContinuousContractBarService::OnSecurityServiceEvent(
  ...
  if (m_pimpl->m_source.OnSecurityServiceEvent(time, security, securityEvent)) {
    m_pimpl->OnNewBar();
    isUpdated = true;
  }
...
ContinuousContractBarService::Bar ContinuousContractBarService::GetBar(
  ...
  if (index >= GetSize()) {
    throw BarDoesNotExistError(
        !GetSize() ? "ContinuousContractBarService is empty"
  ...
  if (index < m_pimpl->m_bars.size()) {
    ...
    return m_pimpl->m_source.GetBarByReversedIndex(
        m_pimpl->m_sizeOfCurrentContract - (index - m_pimpl->m_bars.size()) -
        1);
  }
}

ContinuousContractBarService::Bar
ContinuousContractBarService::GetBarByReversedIndex(size_t index) const {
  if (index >= GetSize()) {
    throw BarDoesNotExistError(
        !GetSize() ? "ContinuousContractBarService is empty"
                   : "Index is out of range of ContinuousContractBarService");
...
ContinuousContractBarService::Bar ContinuousContractBarService::GetLastBar()
    const {
...
CreateContinuousContractBarService(Context &context,
                                   const std::string &instanceName,
                                   const IniSectionRef &configuration) {
  return boost::make_shared<ContinuousContractBarService>(context, instanceName,
                                                          configuration);
}
