...
public:
	...
	bool OnNewValue(const BarService::Bar &bar, const Security &security) {
...

		{
			...
			m_lastValue.pdi
				= (accs::directionalMovement(m_pdmEma) / trStat) * 100;
			m_lastValue.ndi
				= (accs::directionalMovement(m_ndmEma) / trStat) * 100;
		}
...
Adx::Error::Error(const char *what) noexcept
	: Exception(what) {
	//...//
}
...
bool Adx::IsEmpty() const {
	return m_pimpl->m_lastValueNo == 0;
}

Adx::Point Adx::GetLastPoint() const {
	if (IsEmpty()) {
