...
class Stochastic::Implementation : private boost::noncopyable {
 public:
  ...
  accs::accumulator_set<Double, accs::stats<accs::tag::rolling_mean>> m_kAcc;

  const size_t m_periodD;
  accs::accumulator_set<Double, accs::stats<accs::tag::rolling_mean>> m_dAcc;
  ...
  explicit Implementation(Stochastic &self, const IniSectionRef &conf)
    ...
    m_self.GetLog().Info(
        ...
        ", period %3% for %%D.",
        m_highestPrices.capacity(), m_periodSmoothingK, m_periodD);
  ...
  void OpenPointsLog() {
    ...
    log << "Date,Time,Open,High,Low,Close,%K,%D" << std::endl;
    log << std::setfill('0');
  ...
  bool OnNewValue(const BarService::Bar &bar, const Security &security) {
    ...
    const auto &lowestPrice =
        *std::min_element(m_lowestPrices.begin(), m_lowestPrices.end());
    m_lastValue.k = m_lastValue.source.close - lowestPrice;
    {
      const auto &highestPrice =
          *std::max_element(m_highestPrices.begin(), m_highestPrices.end());
...
Stochastic::Error::Error(const char *what) noexcept : Exception(what) {}
...
bool Stochastic::IsEmpty() const { return m_pimpl->m_lastValueNo == 0; }

const Stochastic::Point &Stochastic::GetLastPoint() const {
  if (IsEmpty()) {
    throw ValueDoesNotExistError("Stochastic Oscillator Indicator is empty");
  }
  return m_pimpl->m_lastValue;
}

bool Stochastic::OnServiceDataUpdate(const Service &service,
