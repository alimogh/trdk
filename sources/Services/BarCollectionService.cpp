...
BarCollectionService::MethodDoesNotSupportBySettings::MethodDoesNotSupportBySettings(
		const char *what)
		throw()
	: Error(what) {
	//...//
...
bool BarCollectionService::OnNewBar(const Security &security, const Security::Bar &bar) {
	return m_pimpl->m_onNewBar(security, bar);
}

bool BarCollectionService::OnLevel1Tick(
		const Security &security,
		const pt::ptime &time,
		const Level1TickValue &value) {
...
bool BarCollectionService::OnNewTrade(
		const Security &security,
		const pt::ptime &time,
		const ScaledPrice &price,
		const Qty &qty) {
