...
BollingerBandsService::HasNotHistory::HasNotHistory(const char *what) throw()
    : Error(what) {}
...
size_t LoadPeriodSetting(const IniSectionRef &configuration,
                         BollingerBandsService &service) {
  const auto &result = configuration.ReadTypedKey<uintmax_t>(Keys::period);
  if (result <= 1) {
    service.GetLog().Error(
        "Wrong period specified (%1% frames): must be greater than 1.", result);
...
History *LoadHistorySetting(const IniSectionRef &configuration,
...
class BollingerBandsService::Implementation : private boost::noncopyable {
 public:
  ...
  accs::accumulator_set<
      double,
      accs::stats<accs::tag::StandardDeviationForBb, accs::tag::rolling_mean>>
      m_stat;
 ...
 public:
  explicit Implementation(BollingerBandsService &service,
    ...
    m_service.GetLog().Info(
        ...
        ", channels UUIDs: %7%/%8%.",
        Configuration::Keys::period, m_period, Configuration::Keys::deviation,
        m_deviation, Configuration::Keys::isHistoryOn, m_history ? "yes" : "no",
        m_lowValuesId, m_highValuesId);
    {
  ...
  void LogPoint(const Point &point) {
    ...
    m_pointsLog << point.time.date() << ','
                ...
                << point.source << ',' << point.low << ',' << point.middle
                << ',' << point.high << std::endl;
  }
...
BollingerBandsService::GetHistoryPointByReversedIndex(size_t index) const {
  m_pimpl->CheckHistoryIndex(index);
...
void BollingerBandsService::DropLastPointCopy(
    ...
    const DropCopyDataSourceInstanceId &highValueId) const {
  if (IsEmpty()) {
    throw ValueDoesNotExistError("BollingerBandsService is empty");
  }

  GetContext().InvokeDropCopy([this, &lowValueId,
                               &highValueId](DropCopy &dropCopy) {
    AssertLt(0, m_pimpl->m_lastValueNo);
    const auto recordNo = m_pimpl->m_lastValueNo - 1;
    dropCopy.CopyAbstractData(lowValueId, recordNo, m_pimpl->m_lastValue->time,
...
TRDK_SERVICES_API boost::shared_ptr<trdk::Service> CreateBollingerBandsService(
    ...
    const std::string &instanceName,
    const IniSectionRef &configuration) {
  return boost::make_shared<BollingerBandsService>(context, instanceName,
                                                   configuration);
}
