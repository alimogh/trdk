...
EngineServer::Exception::Exception(const char *what) noexcept : Base(what) {}
...
EngineServer::EngineServer() : m_pimpl(boost::make_unique<Implementation>()) {}
...
void EngineServer::InitLog(const fs::path &logFilePath) {
  ...
  m_pimpl->m_logFile.open(logFilePath.c_str(),
...
EngineServer::EngineId EngineServer::CreateEngine() {
  ...
  settingsString << "[Defaults]\n"
                    ...
                    "module = "
                 << GetDllFileDir().string() << "/RobotEngine\n"
                                                "positions = no\n"
                                                "client_id = "
                 << ++m_pimpl->m_clientId << "\n"
                                             "account = \n";
  ...
  auto context = boost::make_shared<trdk::Engine::Context>(
      GetLog(), m_pimpl->m_tradingLog, Settings(), conf,
...
const SimpleApi::Engine &EngineServer::GetEngine(
    const EngineId &engineId) const {
  return const_cast<EngineServer *>(this)->GetEngine(engineId);
}

EventsLog &EngineServer::GetLog() { return m_pimpl->m_log; }
