...
SimpleApi::Engine &GetEngine() { return GetEngineServer().CheckEngine(0); }

EventsLog &GetLog() { return GetEngineServer().GetLog(); }
}
...
const TradingSystem::Account &GetAccount(const char *account) {
  ...
  try {
    ...
    GetLog().Error("Failed to get Account Info for \"%1%\": \"%2%\".", account,
                   ex);
    throw;
  } catch (...) {
    AssertFailNoException();
...
double _stdcall trdk_GetOptionPremium(const char *symbol,
                                      const char *currency,
                                      const char *exchange,
  ...
  try {
    ...
    try {
      security = &GetEngine().GetOptionSecurity(
          symbol, currency, exchange, static_cast<unsigned int>(expirationDate),
          strike, right, 0);
    } catch (const Exception &ex) {
      GetLog().Error(
          ...
          ", strike: \"%5%\""
          ", right: \"%6%\": \"%7%\".",
          symbol, currency, exchange, expirationDate, strike, right, ex);
      return 0;
    }
...
double _stdcall GetOptionPremium(const char *symbol,
                                 const char *currency,
                                 const char *exchange,
                                 double expirationDate,
                                 double strike,
                                 const char *right,
                                 double requestDate) {
  return trdk_GetOptionPremium(symbol, currency, exchange, expirationDate,
                               strike, right, requestDate);
}
