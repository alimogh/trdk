...
void TriangulationWithDirection::UpdateDirection(const Service &service) {
	...
	auto pairsDataIt = std::find_if(
		m_pairsData.begin(),
		m_pairsData.end(),
		[&service](const PairData &pairData) {
			return pairData.service == &service;
		}); 
	Assert(pairsDataIt != m_pairsData.end());
	...
	for (size_t ecn = 0; !hasNotOpportunity && ecn < ecnsCount; ++ecn) {
 		if (
				!m_useAdjustedBookForTrades
				&& pairsDataIt->service->GetSecurity(ecn).IsBookAdjusted()) {
 			continue;
 		}
		const Security &security = pairsDataIt->service->GetSecurity(ecn);
		{
			const auto &bid = security.GetBidPrice();
			if (IsZero(bid)) {
				hasNotOpportunity = true;
			} else if (pairsDataIt->bestBid.price < bid) {
				pairsDataIt->bestBid.price = bid;
				pairsDataIt->bestBid.source = ecn;
			}
		}
		{
			...
			if (IsZero(ask)) {
				hasNotOpportunity = true;
					...
					pairsDataIt->bestAsk.price > ask
					|| IsZero(pairsDataIt->bestAsk.price)) {
				pairsDataIt->bestAsk.price = ask;
				pairsDataIt->bestAsk.source = ecn;
			}
		}
	}
...
size_t TriangulationWithDirection::CalcBookUpdatesNumber() const {
	size_t result = 0;
	foreach (const auto &pairData, m_pairsData) {
		result += pairData.service->CalcUpdatesNumber();
	}
	return result;
}
