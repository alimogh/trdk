...
struct StateTrait {
  ...
  static bool IsOverloaded(const Value &value,
  ...
  static bool IsExtremum(const Adx::Point &adx,
  ...
  static bool IsDirectionCorrect(const Value &shouldBeUnderIfRising,
...
bool IsOverloaded(const Stochastic::Point &stoch, bool isRising) {
  return isRising ? IsOverloaded<true>(stoch) : IsOverloaded<false>(stoch);
}
bool IsOverloaded(const Adx::Point &adx) { return adx.adx >= 50; }
...
bool IsExtremum(const Adx::Point &adx,
                const History &pdiHistory,
                const History &ndiHistory) {
...
bool IsDirectionCorrect(const Value &shouldBeUnderIfRising,
                        const Value &shouldBeAboveIfRising) {
  return StateTrait<isRising>::IsDirectionCorrect(shouldBeUnderIfRising,
...
bool IsDirectionCorrect(const Adx::Point &adx, bool isRising) {
  return isRising ? IsDirectionCorrect<true>(adx)
...
bool IsDirectionCorrect(const Stochastic::Point &stoch, bool isRising) {
  return isRising ? IsDirectionCorrect<true>(stoch)
...
bool IsDirectionStrong(const Adx::Point &adx, bool isRinisg) {
  return isRinisg ? IsDirectionStrong<true>(adx)
...
bool IsTimeToOpenPositions(const pt::ptime &time) {
...

  {
    static std::pair<pt::time_duration, pt::time_duration> night =
        std::make_pair(pt::time_duration(23, 40, 0),
  ...
  {
    static std::pair<pt::time_duration, pt::time_duration> day = std::make_pair(
        pt::time_duration(13, 55, 0), pt::time_duration(14, 05, 0));
...

  {
    static std::pair<pt::time_duration, pt::time_duration> evenging =
        std::make_pair(pt::time_duration(18, 35, 0),
...
struct Settings {
  struct Order {
    ...
    explicit Order(bool isOpen, const IniSectionRef &conf)
        : passiveMaxLifetime(ReadOrderMaxLifetime(
              conf,
              isOpen ? "open.passive_order_max_lifetime.sec"
                     : "close.passive_order_max_lifetime.sec")),
          maxPriceDelta(conf.ReadTypedKey<double>(
              isOpen ? "open.order_price_max_delta"
   ...
   private:
    static pt::time_duration ReadOrderMaxLifetime(const IniSectionRef &conf,
                                                  const char *key) {
...
class Trend {
 private:
  class History : private boost::noncopyable {
   public:
    explicit History(size_t size) : m_buffer(size) {
      AssertLe(1, m_buffer.capacity());
    }

   public:
    operator bool() const { return m_buffer.full(); }
 ...
 public:
  //! Actual trend.
  /** @return True, if trend is "rising", false if trend is "falling", not true
    *         and not false if there is no trend at this moment.
    */
  const boost::tribool &IsRising() const { return m_isRising; }

  bool IsDivergence() const { return m_isDivergenceSignal; }

  //! Updates trend info.
  /** @return True, if trend is detected, or changed, or lost.
    */
  bool Update(const BollingerBands::Point &price,
              const Adx::Point &adx,
              const Rsi::Point &rsi,
  ...
  const char *GetTrend() const {
    return m_isRising ? "rising" : !m_isRising ? "falling" : "no trend";
  }

  const char *GetBound() const {
    return m_bound ? "upper" : !m_bound ? "lower" : "?";
  }

  double GetOutOfBoundsStat() const {
    double result = double(m_numberOfUpdatesOutsideBounds);
    result /= m_numberOfUpdatesOutsideBounds + m_numberOfUpdatesInsideBounds;
    result *= 100;
    return result;
  }

  const char *GetLastTrendCheckResult() const { return m_lastTrendCheckResult; }
 ...
 private:
  ...
  boost::tribool CheckOutside(const Adx::Point &adx,
                              const Rsi::Point &rsi,
    ...
    if ((m_bound == boost::tribool(isUpper) || m_isDivergenceTrend) &&
    ...
    if (!boost::indeterminate(m_bound) && m_bound != boost::tribool(isUpper)) {
    ...
    if (!iu::IsOverloaded<isUpper>(rsi)) {
      ...
      if (isUpper) {
        if (iu::IsExtremum<true>(70, m_rsiHistory)) {
          m_lastDivergenceCheckResult = "out->rsi->was-not->overbought";
        } else {
          m_lastDivergenceCheckResult = "out->rsi->was-overbought";
          return false;
        }
  ...
  boost::tribool CheckInside(const Adx::Point &adx,
                             const Rsi::Point &rsi,
                             const Stochastic::Point &stoch) {
    ...
    if (!boost::indeterminate(m_isRising) && boost::indeterminate(result)) {
  ...
  boost::tribool CheckInsideTrendCompletion(const Adx::Point &adx,
                                            const Rsi::Point &rsi,
                                            const Stochastic::Point &stoch) {
    if (boost::indeterminate(m_isRising)) {
    ...
    if (m::round(adx.adx.Get()) <= 25) {
      m_lastTrendCheckResult =
          !m_isDivergenceTrend ? "in->adx->weak" : "in->divergence->adx->weak";
      return boost::indeterminate;
    }
    ...
    if (!m_isDivergenceTrend) {
      ...
      if (!iu::IsOverloaded(rsi, m_isRising) && !iu::IsOverloaded(adx)) {
  ...
  boost::tribool CheckInsideDivergence(const Rsi::Point &rsi,
                                       const Stochastic::Point &stoch) {
    Assert(!boost::indeterminate(m_isRising));
    if (boost::indeterminate(m_isRising)) {
    ...
    if (!iu::IsOverloaded(rsi, result) && !iu::IsOverloaded(stoch, result)) {
      if (m_isRising) {
        if (iu::IsExtremum<true>(70, m_rsiHistory)) {
          m_lastDivergenceCheckResult = "rsi->was-not->overbought";
          return boost::indeterminate;
        }
        if (iu::IsExtremum<true>(80, m_kHistory)) {
          m_lastDivergenceCheckResult = "stoch->was-not->overbought";
          return boost::indeterminate;
        }
      } else if (iu::IsExtremum<false>(30, m_rsiHistory)) {
 ...
 private:
  ...
  bool IsExtremum(const Stochastic::Point &stoch, bool isRising) const {
    return isRising ? IsExtremum<true>(stoch) : IsExtremum<false>(stoch);
  }
...
class Strategy : public trdk::Strategy {
 ...
 public:
  explicit Strategy(Context &context,
      ...
      : Base(context,
        ...
        m_prices(nullptr),
        m_pricesServiceDropCopyIds(DropCopy::nDataSourceInstanceId,
 ...
 protected:
  virtual void OnSecurityStart(Security &security,
                               Security::Request &request) override {
    if (!m_security) {
      m_settings.OnSecurity(security);
      m_security = &security;
      GetLog().Info("Using \"%1%\" to trade...", *m_security);
    } else if (m_security != &security) {
      throw Exception("Strategy can not work with more than one security");
    }
    Base::OnSecurityStart(security, request);
  }

  virtual void OnServiceStart(const Service &service) override {
    const auto *const bb = dynamic_cast<const BollingerBands *>(&service);
    ...
    const auto *const stochastic = dynamic_cast<const Stochastic *>(&service);
    if (bb) {
      OnBbServiceStart(*bb);
    }
    if (bars) {
  ...
  virtual void OnServiceDataUpdate(
      const Service &service, const Milestones &delayMeasurement) override {
    if (m_bars == &service) {
    ...
    if (m_prices == &service) {
      m_prices->DropLastPointCopy(m_pricesServiceDropCopyIds.first,
 ...
 private:
  void OnBarServiceStart(const BarService &service) {
    ...
    GetContext().InvokeDropCopy(
        [this, &service, &dropCopySourceId](DropCopy &dropCopy) {
          m_barServiceDropCopyId = dropCopy.RegisterDataSourceInstance(
              *this, service.GetTypeId(), service.GetId());
          dropCopySourceId =
              boost::lexical_cast<std::string>(m_barServiceDropCopyId);
        });

    m_bars = &service;

    GetLog().Info("Using bars service \"%1%\" (drop copy ID: %2%)...", *m_bars,
                  dropCopySourceId);
  }

  void OnBbServiceStart(const BollingerBands &service) {
    if (m_prices) {
      throw Exception("Strategy uses one prices service, but configured more");
    }
    AssertEq(DropCopy::nDataSourceInstanceId, m_pricesServiceDropCopyIds.first);
    AssertEq(DropCopy::nDataSourceInstanceId,
             m_pricesServiceDropCopyIds.second);

    auto dropCopySourceId =
        std::make_pair<std::string, std::string>("not used", "not used");
    GetContext().InvokeDropCopy([this, &service,
                                 &dropCopySourceId](DropCopy &dropCopy) {
      m_pricesServiceDropCopyIds.first = dropCopy.RegisterDataSourceInstance(
          *this, service.GetTypeId(), service.GetLowValuesId());
      m_pricesServiceDropCopyIds.second = dropCopy.RegisterDataSourceInstance(
          *this, service.GetTypeId(), service.GetHighValuesId());
      dropCopySourceId = std::make_pair(
          boost::lexical_cast<std::string>(m_pricesServiceDropCopyIds.first),
          boost::lexical_cast<std::string>(m_pricesServiceDropCopyIds.second));
    });

    m_prices = &service;

    GetLog().Info("Using price service \"%1%\" (drop copy IDs: %2%/%3%)...",
                  *m_prices, dropCopySourceId.first, dropCopySourceId.second);
  ...
  void OnAdxServiceStart(const Adx &service) {
    if (m_adx) {
      throw Exception("Strategy uses one ADX service, but configured more");
    }
    m_adx = &service;
  ...
  void OnRsiServiceStart(const Rsi &service) {
    if (m_rsi) {
      throw Exception("Strategy uses one RSI service, but configured more");
    }
    m_rsi = &service;
  ...
  void UpdateStat() {
    ...
    const auto delta = position.IsLong() ? close - open : open - close;
    if (delta > 0) {
  ...
  void CheckOrder(Position &position, const Milestones &delayMeasurement) {
    Assert(position.HasActiveOrders());
    AssertLt(0, position.GetNumberOfOpenOrders());
    ...
    if (position.HasActiveOpenOrders()) {
      ...
      startPrice = m_settings.open.passiveMaxLifetime != pt::not_a_date_time &&
      ...
      AssertNe(CLOSE_REASON_NONE, position.GetCloseReason());
      startPrice = m_settings.close.passiveMaxLifetime != pt::not_a_date_time &&
      ...
      if (actualPrice >= startPrice + priceAllowedDelta) {
        ...
        GetTradingLog().Write(
            ...
            [&](TradingRecord &record) {
              record % m_security->DescalePrice(startPrice) %
                  m_security->DescalePrice(priceAllowedDelta) %
                  m_security->DescalePrice(startPrice + priceAllowedDelta) %
                  m_security->DescalePrice(actualPrice) % m_trend.GetTrend() %
      ...
      GetTradingLog().Write(
          ...
          [&](TradingRecord &record) {
            record % m_security->DescalePrice(startPrice) %
                ...
                m_security->DescalePrice(startPrice - priceAllowedDelta) %
                m_security->DescalePrice(actualPrice) % m_trend.GetTrend() %
                m_security->GetBidPriceValue() % m_security->GetAskPriceValue();
          });
      try {
        Verify(position.CancelAllOrders());
      } catch (const TradingSystem::UnknownOrderCancelError &ex) {
    ...
    if (allowedTime != pt::not_a_date_time) {
      const auto endTime = position.GetActiveOrderTime() + allowedTime;
      if (endTime <= GetContext().GetCurrentTime()) {
        ...
        GetTradingLog().Write(
            ...
            "\tbid/ask=%5$.2f/%6$.2f",
            [&](TradingRecord &record) {
              record % position.GetActiveOrderTime().time_of_day() %
                  endTime.time_of_day() %
                  (endTime - position.GetActiveOrderTime()) %
                  m_trend.GetTrend() % m_security->GetBidPriceValue() %
                  m_security->GetAskPriceValue();
            });
  ...
  void CheckSignal(const Milestones &delayMeasurement) {
    ...
    if (!position) {
      ...
      position = &OpenPosition(m_trend.IsRising(), price, delayMeasurement);
  ...
  Position &OpenPosition(bool isLong, const Milestones &delayMeasurement) {
    Assert(m_prices);
    return OpenPosition(isLong, m_prices->GetLastPoint(), delayMeasurement);
  }

  Position &OpenPosition(bool isLong,
                         const BollingerBands::Point &price,
                         const Milestones &delayMeasurement) {
    boost::shared_ptr<Position> position =
        isLong ? CreatePosition<LongPosition>(m_security->GetAskPriceScaled(),
                                              delayMeasurement)
               : CreatePosition<ShortPosition>(m_security->GetBidPriceScaled(),
                                               delayMeasurement);
    ...
    GetContext().InvokeDropCopy([this, &position](DropCopy &dropCopy) {
      dropCopy.ReportOperationStart(*this, position->GetId(),
                                    GetContext().GetCurrentTime());
    });

    ContinuePosition(*position);

    m_stat.maxProfitPriceDelta = m_stat.maxLossPriceDelta = 0;
    m_stops.Reset(price);
      ...
      case CLOSE_REASON_SIGNAL:
        if (!boost::indeterminate(m_trend.IsRising()) &&
          ...
          GetTradingLog().Write(
              ...
              [&](TradingRecord &record) { record % m_trend.GetTrend(); });
        ...
        break;
      default:
        GetTradingLog().Write("reopening\tcanceled by prev. close reason %1%",
                              [&](TradingRecord &record) {
  ...
  boost::shared_ptr<Position> CreatePosition(
      const ScaledPrice &price, const Milestones &delayMeasurement) {
    return boost::make_shared<PositionType>(
        *this, m_generateUuid(), 1,
        GetTradingSystem(m_security->GetSource().GetIndex()), *m_security,
        m_security->GetSymbol().GetCurrency(), m_settings.qty, price,
        delayMeasurement);
  }
  ...
  void LogTrend(const char *action,
                const BollingerBands::Point &price,
                const Adx::Point &adx,
                const Rsi::Point &rsi,
                const Stochastic::Point &stochastic) {
          ...
          record % action                              // 1
              ...
              (price.source > price.high
                   ...
                   : price.source > price.middle
                         ? "upper"
                         : price.source < price.low ? "LOWEST" : "lower")  // 7
              ...
              %
              (adx.adx < 25 ? adx.pdi > adx.ndi ? "WEAK rising" : "WEAK falling"
                            : adx.adx >= 75
                                  ? adx.pdi > adx.ndi ? "EXTREME rising"
                                                      : "EXTREME falling"
                                  : adx.adx >= 50
                                        ? adx.pdi > adx.ndi ? "STRONG rising"
                                                            : "STRONG falling"
                                        : adx.pdi > adx.ndi ? "rising"
                                                            : "falling")  // 14
              % adx.adx                                                   // 15
              % adx.pdi                                                   // 16
              % adx.ndi                                                   // 17
              % (stochastic.d > 80 || stochastic.k > 80
                     ? stochastic.k > stochastic.d ? "OVERBOUGHT rising"
                                                   : "OVERBOUGHT falling"
                     : stochastic.d < 20 || stochastic.k < 20
                           ? stochastic.k < stochastic.d ? "OVERSOLD falling"
                                                         : "OVERSOLD rising"
                           : stochastic.k > stochastic.d ? "rising"
                                                         : "falling")  // 18
              % stochastic.d                                           // 19
              % stochastic.k                                           // 20
              %
  ...
  void ReportOperation(Position &pos) {
    ...
    pos.IsProfit() ? ++m_stat.numberOfWinners : ++m_stat.numberOfLosers;

    try {
      GetContext().InvokeDropCopy([this, &pos, &pnlVolume,
                                   &pnlRatio](DropCopy &dropCopy) {
        FinancialResult financialResult;
        financialResult.emplace_back(
            std::make_pair(m_security->GetSymbol().GetCurrency(), pnlVolume));
        dropCopy.ReportOperationEnd(
            pos.GetId(), pos.GetCloseTime(), pos.GetCloseReason(),
            !pos.IsProfit() ? OPERATION_RESULT_LOSS : OPERATION_RESULT_PROFIT,
            pnlRatio, std::move(financialResult));
      });
    } catch (const std::exception &ex) {
      GetLog().Error("Failed to report operation end: \"%1%\".", ex.what());
    } catch (...) {
    ...
    m_strategyLog << pos.GetOpenStartTime().date() << ','
                  ...
                  << ',' << ExcelTextField(pos.GetCloseTime() -
                                           pos.GetCloseStartTime())
                  << ','
                  << ExcelTextField(pos.GetCloseTime() - pos.GetOpenTime())
                  << ',' << pos.GetType() << ',' << pnlVolume << ',' << pnlRatio
                  << ',' << m_stat.pnl;
    {
      const auto delta = m_security->DescalePrice(m_stat.maxProfitPriceDelta +
    ...
    {
      const auto delta =
          std::abs(m_security->DescalePrice(m_stat.maxLossPriceDelta));
      m_strategyLog << ',' << (delta * m_security->GetQuoteSize());
    }
    m_strategyLog
        << (pos.IsProfit() ? ",1,0" : ",0,1") << ',' << m_stat.numberOfWinners
        << ',' << m_stat.numberOfLosers << ','
        << ((double(m_stat.numberOfWinners) / numberOfOperations) * 100) << ','
        << ((double(m_stat.numberOfLosers) / numberOfOperations) * 100) << ','
        << pos.GetOpenedQty() << ','
        << (!m_stat.isPrevSignalIsDivergence ? "trend" : "divergence") << ','
  ...
  struct Stops {
    ...
    void Update(const BollingerBands::Point &price) {
      ...
      *takeProfit = TakeProfit::Params{
          takeProfit->GetMinProfitPerLotToActivate(), width * 0.6};
    }
...
boost::shared_ptr<Strategy> CreateStrategy(Context &context,
  ...
  return boost::make_shared<IntradayTrend::Strategy>(context, instanceName,
                                                     conf);
}
