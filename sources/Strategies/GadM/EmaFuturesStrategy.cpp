...
namespace EmaFuturesStrategy {
	...
	public:
		...
		virtual void OnServiceStart(const Service &service) {
			...
			if (maService) {
				...
				if (boost::iequals(maService->GetTag(), "fastema")) {
					GetLog().Info(
				...
				} else if (boost::iequals(maService->GetTag(), "slowema")) {
					GetLog().Info(
	...
	private:
		...
		void CheckOppositePositionOpenSignal(const Position &prevPosition) {
			...
			if (prevPosition.GetCloseReason() != CLOSE_REASON_NONE) {
				return;
			}
		...
		void FinishRollOver(Position &oldPosition) {
			...
			if (
					...
					|| oldPosition.GetCloseReason() != CLOSE_REASON_ROLLOVER) {
				return;
			}
