...
namespace {
	...
	void TestCleanPriceBookSide(BookSide &side) {
		...
		EXPECT_THROW(
			side.GetLevel(trdk::PriceBook::GetSideMaxSize() - 1),
			trdk::Lib::LogicError);
		EXPECT_THROW(
			side.GetLevel(trdk::PriceBook::GetSideMaxSize()),
			trdk::Lib::LogicError);
...
public:
	...
	void GeneratePriceUpdates(Side &side) {
		...
		while (m_source.size() < m_size) {
			...
			ASSERT_EQ(
				index < trdk::PriceBook::GetSideMaxSize(),
				pos->second.isUsed);
	...
	void UpdateQty(Side &side, const trdk::Qty &qty) {
		...
		foreach (auto &level, m_source) {
			...
			EXPECT_EQ(
				++i <= trdk::PriceBook::GetSideMaxSize(),
