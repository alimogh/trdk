...
class Terminal::Implementation : private boost::noncopyable {
 private:
  ...
  class OrderCommand : public Command,
   ...
   protected:
    enum OrderTime { ORDER_TIME_NOT_SET, ORDER_TIME_IOC, ORDER_TIME_DAY };

   public:
    explicit OrderCommand(TradingSystem &tradingSystem,
   ...
   protected:
    bool ParseSymbol(const std::string &field) {
      ...
      m_security = m_tradingSystem.GetContext()
    ...
    void OnReply(const OrderId &orderId,
  ...
  class SellCommand : public OrderCommand {
   public:
    explicit SellCommand(TradingSystem &tradingSystem,
                         RiskControlScope &riskControlScope)
  ...
  class BuyCommand : public OrderCommand {
   public:
    explicit BuyCommand(TradingSystem &tradingSystem,
                        RiskControlScope &riskControlScope)
        : OrderCommand(tradingSystem, riskControlScope) {}
    virtual ~BuyCommand() {}
    virtual void Execute() {
  ...
  class TestCommand : public Command,
   ...
   public:
    ...
    virtual void Execute() { m_tradingSystem.Test(); }
  };
 ...
 public:
  explicit Implementation(const fs::path &cmdFile, TradingSystem &tradingSystem)
      : m_cmdFile(cmdFile),
        ...
        m_lastSeqnumber(0) {
    m_tradingSystem.GetLog().Info(
        "Terminal: Starting with commands file %1%...", m_cmdFile);
    m_notificator.Start();
  }
  ...
  void OnCmdFileChanged() {
    ...
    if (!f) {
      m_tradingSystem.GetLog().Error(
          "Terminal: Failed to open terminal commands file %1%.", m_cmdFile);
      return;
    }
    ...
    if (line >= 0) {
      m_tradingSystem.GetLog().Debug(
          ...
          m_cmdFile, line, execsCount);
    }
  }

  bool ProcessCmdLine(size_t lineNo,
                      const char *line,
    ...
    for (It it = boost::make_split_iterator(
             line, boost::token_finder(boost::algorithm::is_space(),
      ...
      if (!seqnumber) {
        ...
        if (boost::iequals(field, "buy")) {
          command.reset(new BuyCommand(m_tradingSystem, m_riskControlScope));
        } else if (boost::iequals(field, "sell")) {
          command.reset(new SellCommand(m_tradingSystem, m_riskControlScope));
          ...
          m_tradingSystem.GetLog().Error(
              ...
              " Must be %4%.",
              field, m_cmdFile, lineNo, commandsHelp);
          return false;
        }
        ...
        try {
          ...
          m_tradingSystem.GetLog().Error(
              ...
              " at \"%2%\":%3%. %4%.",
              field, m_cmdFile, lineNo, ex.what());
          return false;
        }
      }
    }
    ...
    if (!command) {
      m_tradingSystem.GetLog().Error(
          "Terminal: No command set at %1%:%2%. Must be %3%.", m_cmdFile,
          lineNo, commandsHelp);
      return false;
    }
    ...
    if (!commandErrorText.empty()) {
      m_tradingSystem.GetLog().Error(
          ...
          m_cmdFile, lineNo, commandErrorText);
      return false;
    }
...
Terminal::~Terminal() { delete m_pimpl; }
