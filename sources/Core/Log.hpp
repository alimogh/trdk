...
class TRDK_CORE_API Log : private boost::noncopyable {
 ...
 public:
  ...
  void DisableStream() noexcept { m_isStreamEnabled = false; }

  void EnableStdOut() { m_isStdOutEnabled = true; }

  void DisableStdOut() noexcept { m_isStdOutEnabled = false; }
 ...
 public:
  boost::posix_time::ptime GetTime() const {
    ...
    return lt::local_microsec_clock::local_time(GetTimeZone()).local_time();
  }
 ...
 private:
  template <typename Message>
  static void AppendMessage(const char *tag,
                            const boost::posix_time::ptime &time,
                            const ThreadId &theadId,
                            const std::string *module,
                            const Message &message,
  ...
  static void AppendRecordHead(const char *tag,
                               const boost::posix_time::ptime &,
                               const ThreadId &,
                               const std::string *module,
                               std::ostream &);
  static void AppendRecordEnd(std::ostream &os) { os << std::endl; }

  template <typename Message>
  static void AppendString(const Message &message, std::ostream &os) {
    os << message;
  }
