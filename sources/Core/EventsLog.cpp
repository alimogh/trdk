...
void EventsLog::BroadcastCriticalError(const std::string &message) noexcept {
  try {
    ...
    std::cerr << "Unhandled Exception in " << __FUNCTION__ << " (" << __FILE__
              << ':' << __LINE__ << ')' << std::endl;
#ifdef BOOST_WINDOWS
    raise(SIGABRT);
#else
...
struct UnhandledExceptionMessage : private boost::noncopyable {
 ...
 private:
  ...
  void Format(boost::format &format,
...
void EventsLog::BroadcastUnhandledException(const char *function,
                                            const char *file,
  ...
  try {
    ...
    try {
      ...
      message.CreateStandard("LOCAL", ex, function, file, line);
    } catch (const std::exception &ex) {
      message.CreateStandard("STANDART", ex.what(), function, file, line);
    } catch (int) {
...
ModuleEventsLog::ModuleEventsLog(const std::string &name, EventsLog &log)
