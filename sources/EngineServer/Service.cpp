...
EngineServer::Service::Config::Config(const fs::path &configPath)
  ...
  timeout =
      pt::seconds(long(section.ReadTypedKey<size_t>("tws_timeout_seconds")));
...
EngineServer::Service::Connection::Connection(io::io_service &io,
                                              const Topics &topics,
                                              EventsLog &log,
                                              bool isWampDebugOn)
    : log(log),
...
void EngineServer::Service::Connection::ScheduleNextCurrentTimeNotification() {
  ...
  currentTimePublishTimer.async_wait([weakSelf](const sys::error_code &error) {
    ...
    sharedSelf->session->publish(sharedSelf->topics.time,
...
void EngineServer::Service::Connection::ScheduleIoTimeout(
  ...
  ioTimeoutTimer.async_wait(
      [weakSelf, startTime, timeout](const sys::error_code &error) {
        ...
        sharedSelf->log.Warn("TWS IO operation time out: %1% -> %2% (%3%).",
                             startTime, sharedSelf->log.GetTime(), timeout);
...
void EngineServer::Service::DropCopy::OpenDataLog(const fs::path &logsDir) {
  ...
  m_dataLogFile.open(dataLogFilePath.string().c_str(),
                     std::ios::out | std::ios::ate | std::ios::app);
  if (!m_dataLogFile) {
    GetLog().Error("Failed to open file %1% for Drop Copy logging.",
                   dataLogFilePath);
...
void EngineServer::Service::DropCopy::Start() { m_queue.Start(); }

void EngineServer::Service::DropCopy::Stop(bool sync) { m_queue.Stop(sync); }
...
void EngineServer::Service::DropCopy::Flush() { m_queue.Flush(); }

void EngineServer::Service::DropCopy::Dump() { m_queue.Dump(); }
...
EngineServer::Service::DropCopy::ContinueStrategyInstance(
    const Strategy &strategy, const pt::ptime &time) {
...
EngineServer::Service::DropCopy::RegisterDataSourceInstance(
    const Strategy &strategy, const uuids::uuid &type, const uuids::uuid &id) {
...
void EngineServer::Service::DropCopy::CopyTrade(
  ...
  m_queue.Enqueue(
      [this, time, tradingSystemTradeId, orderId, price, qty, bestBidPrice,
       ...
       bestAskQty](size_t recordNumber, size_t attemptNo, bool dump) -> bool {
        return m_service.StoreTrade(
            recordNumber, attemptNo, dump, time, tradingSystemTradeId, orderId,
            price, qty, bestBidPrice, bestBidQty, bestAskPrice, bestAskQty);
      });
}

void EngineServer::Service::DropCopy::ReportOperationStart(
    const Strategy &strategy, const uuids::uuid &id, const pt::ptime &time) {
  m_queue.Enqueue([this, &strategy, id, time](size_t recordNumber,
                                              size_t attemptNo, bool dump) {
    return m_service.StoreOperationStartReport(recordNumber, attemptNo, dump,
                                               strategy, id, time);
  });
}
...
void EngineServer::Service::DropCopy::ReportOperationEnd(
  ...
  m_queue.Enqueue(
      [this, id, time, reason, result, pnl, financialResultPtr](
          size_t recordNumber, size_t attemptNo, bool dump) -> bool {
        return m_service.StoreOperationEndReport(recordNumber, attemptNo, dump,
                                                 id, time, reason, result, pnl,
                                                 financialResultPtr);
      });
}

void EngineServer::Service::DropCopy::CopyBook(const Security &security,
  ...
  m_queue.Enqueue([this, &security, book](size_t recordNumber, size_t attemptNo,
                                          bool dump) -> bool {
    return m_service.StoreBook(recordNumber, attemptNo, dump, security, book);
  });
}
...
EngineServer::Service::Service(const fs::path &engineConfigFilePath,
  ...
  try {
    settings = boost::make_unique<trdk::Settings>(
        IniFile(m_config.path), pt::microsec_clock::universal_time());
  } catch (const std::exception &ex) {
    std::cerr << "Failed to read settings: \"" << ex.what() << "\"."
              << std::endl;
    throw;
  }
  ...
  m_logSubscription = m_log->Subscribe(
      boost::bind(&Service::PublishMessage, this, _1, _2, _3, _4));

  const auto logsDir = settings->GetLogsRootDir();
  fs::create_directories(logsDir);

  const auto logFilePath = logsDir / "service.log";
  m_logFile.open(logFilePath.string().c_str(),
                 std::ios::out | std::ios::ate | std::ios::app);
  if (!m_logFile) {
  ...
  GetLog().Info("Loaded engine from %1% for engine service \"%2%\".",
                m_config.path, m_config.name);
...
boost::shared_ptr<EngineServer::Service::Connection>
EngineServer::Service::Connect(const pt::time_duration &startDelay) {
  GetLog().Debug("Connecting to TWS at %1%:%2%...", m_config.twsHost,
                 m_config.twsPort);
  auto connection = boost::make_shared<Connection>(*m_io, m_topics, GetLog(),
                                                   m_config.wampDebug);
  connection->ScheduleIoTimeout(m_config.timeout);
  connection->socket.async_connect(
      io::ip::tcp::endpoint(io::ip::address::from_string(m_config.twsHost),
                            m_config.twsPort),
...
void EngineServer::Service::OnConnected(
    ...
    const pt::time_duration &startDelay) {
  if (error) {
    GetLog().Error("Failed to connect to TWS at %1%:%2%: \"%3%\" (%4%).",
                   m_config.twsHost, m_config.twsPort, error.message(),
                   error.value());
  ...
  connection->sessionStartFuture = connection->session->start().then(
      [this, connection, startDelay](boost::future<bool> isStartedFuture) {
        ...
        try {
          ...
          GetLog().Error("Failed to start TWS session at \"%1%:%2%\": \"%3%\".",
                         m_config.twsHost, m_config.twsPort, ex.what());
        }
        m_io->post([this, connection, isStarted, startDelay]() {
...
void EngineServer::Service::OnSessionStarted(
  ...
  connection->ScheduleIoTimeout(m_config.timeout);
  connection->sessionJoinFuture = connection->session->join("tws").then(
      [this, connection, startDelay](boost::future<uint64_t> sessionIdFuture) {
        connection->StopIoTimeout();
        ...
        m_io->post([this, connection, sessionId, startDelay]() {
...
void EngineServer::Service::OnSessionJoined(
  ...
  GetLog().Info("Joined to TWS session %1% at \"%2%:%3%\".", *sessionId,
                m_config.twsHost, m_config.twsPort);
  if (!startDelay.is_not_a_date_time()) {
    GetLog().Info("Waiting %1% for the engine instance registration...",
                  startDelay);
    boost::this_thread::sleep(startDelay);
    GetLog().Debug("Registering...");
  }

  const auto &engineInfo = std::make_tuple(m_config.id, m_config.name,
                                           std::string(TRDK_BUILD_IDENTITY));

  connection->ScheduleIoTimeout(m_config.timeout);
  connection->engineRegistrationFuture =
      connection->session
          ->call(connection->topics.registerEngine, engineInfo,
                 m_config.callOptions)
          .then([this, connection, sessionId,
                 engineInfo](boost::future<ab::wamp_call_result> callResult) {
            connection->StopIoTimeout();
            ...
            try {
              ...
              GetLog().Error(
                  ...
                  " with ID \"%3%\": \"%4%\".",
                  std::get<1>(engineInfo), std::get<2>(engineInfo),
                  std::get<0>(engineInfo), ex.what());
            }
            m_io->post([this, connection, sessionId, instanceId, engineInfo]() {
              OnEngineRegistered(connection, *sessionId, instanceId,
                                 engineInfo);
            });
          });
}
...
void EngineServer::Service::OnEngineRegistered(
  ...
  connection->session->publish(
      ...
      std::make_tuple(m_config.id, *instanceId, m_config.name,
                      std::string(TRDK_BUILD_IDENTITY), int(engineState),
                      sessionId));
  ...
  GetLog().Info(
      ...
      *instanceId, std::get<1>(engineInfo), std::get<2>(engineInfo),
...
void EngineServer::Service::RepeatReconnection(
  ...
  GetLog().Warn(
      ...
      prevReconnectError.what(), sleepTime, m_numberOfReconnects);

  timer->async_wait([this, timer](const sys::error_code &error) {
    if (error) {
      GetLog().Debug("Reconnect canceled: \"%1%\" (%2%).", error.message(),
                     error.value());
      return;
    }
    Connect();
  ...
  m_thread = boost::thread([this]() {
    ...
    for (;;) {
      try {
        ...
        GetLog().Error("TWS IO task stopped with error: \"%1%\".", ex.what());
        m_connectionCondition.notify_all();
        throw;
      } catch (const sys::system_error &ex) {
        GetLog().Error("TWS IO task stopped with error: \"%1%\" (%2%).",
                       ex.code().message(), ex.code().value());
        m_connectionCondition.notify_all();
...
void EngineServer::Service::PublishState() const {
  ...
  m_connection->session->publish(m_connection->topics.state,
  ...
  m_engineTask.Start([this]() {
    ...
    try {
      ...
      auto engine = boost::make_shared<Engine::Engine>(
          ...
          *m_dropCopy,
          [this, &engineLogSubscription](trdk::Engine::Context::Log &log) {
          ...
          boost::unordered_map<std::string, std::string>());
      {
        ...
        if (m_engineState != ENGINE_STATE_STARTED) {
          GetLog().Warn("Failed to start engine. Engine state is %1%.",
                        m_engineState);
          return;
        }
...
void EngineServer::Service::StopDropCopy() {
  ...
  if (m_dropCopyTask.IsActive()) {
    ...
    throw Exception("Service performs task which can change Drop Copy state");
  }
  try {
    m_dropCopy->Stop(false);
    m_dropCopyTask.Start([this]() {
...
void EngineServer::Service::OnContextStateChanged(
    const Context::State &state, const std::string *updateMessage) {
  const EngineLock lock(m_engineMutex);
  ...
  switch (state) {
    case Context::STATE_ENGINE_STARTED:
      ...
      !updateMessage
          ...
          : GetLog().Info("Engine changed state to \"started\": \"%s\".",
                          *updateMessage);
...

      {
        const char *logRecord =
            state == Context::STATE_DISPATCHER_TASK_STOPPED_GRACEFULLY
                ? !updateMessage ? "Engine dispatcher task stopped."
        ...
        if (m_engineState == ENGINE_STATE_STOPPING) {
          !updateMessage ? GetLog().Debug(logRecord)
                         : GetLog().Debug(logRecord, *updateMessage);
        } else if (state == Context::STATE_DISPATCHER_TASK_STOPPED_ERROR) {
          !updateMessage ? GetLog().Warn(logRecord)
                         : GetLog().Warn(logRecord, *updateMessage);
        } else {
          !updateMessage ? GetLog().Info(logRecord)
    ...
    case Context::STATE_STRATEGY_BLOCKED:
      ...
      !updateMessage
          ...
          : GetLog().Warn("Engine notified about blocked strategy: \"%s\".",
                          *updateMessage);
...
DropCopyStrategyInstanceId EngineServer::Service::RegisterStrategyInstance(
  ...
  const auto &result = Request<DropCopyStrategyInstanceId>(
      m_dropCopy->TakeRecordNumber(), 1, &Topics::registerStrategyInstance,
      record);
...
DropCopyStrategyInstanceId EngineServer::Service::ContinueStrategyInstance(
  ...
  const auto &result = Request<DropCopyStrategyInstanceId>(
      m_dropCopy->TakeRecordNumber(), 1, &Topics::continueStrategyInstance,
      record);
  if (result == DropCopy::nStrategyInstanceId) {
...
DropCopyDataSourceInstanceId EngineServer::Service::RegisterDataSourceInstance(
  ...
  const auto &result = Request<DropCopyDataSourceInstanceId>(
      m_dropCopy->TakeRecordNumber(), 1, &Topics::registerDataSourceInstance,
      record);
  if (result == DropCopy::nDataSourceInstanceId) {
    throw DropCopy::Exception("Failed to register TWS data source instance");
  }

  return result;
}

bool EngineServer::Service::StoreOperationStartReport(size_t recordNumber,
  ...
  return StoreRecord(&Topics::storeOperationStart, recordNumber, storeAttemptNo,
                     dump, record);
}
...
bool EngineServer::Service::StoreOperationEndReport(
  ...
  return StoreRecord(&Topics::storeOperationEnd, recordNumber, storeAttemptNo,
                     dump, record);
}

bool EngineServer::Service::StoreOrder(size_t recordNumber,
                                       size_t storeAttemptNo,
                                       bool dump,
  ...
  return StoreRecord(&Topics::storeOrder, recordNumber, storeAttemptNo, dump,
                     record);
}

bool EngineServer::Service::StoreTrade(size_t recordNumber,
                                       size_t storeAttemptNo,
                                       bool dump,
                                       const pt::ptime &time,
                                       const std::string &tradingSystemTradeId,
  ...
  return StoreRecord(&Topics::storeTrade, recordNumber, storeAttemptNo, dump,
                     record);
}
...
bool EngineServer::Service::StoreAbstractData(
  ...
  return StoreRecord(&Topics::storeAbstractData, recordNumber, storeAttemptNo,
                     dump, record);
}
...
bool EngineServer::Service::StoreBar(
  ...
  return StoreRecord(&Topics::storeBar, recordNumber, storeAttemptNo, dump,
                     record);
}

bool EngineServer::Service::StoreRecord(const std::string Topics::*topic,
                                        size_t recordNumber,
                                        size_t storeAttemptNo,
                                        bool dump,
  ...
  if (storeAttemptNo == 1) {
    m_dropCopy->GetDataLog().Info("store\t%1%\t%2%\t%3%\t%4%", recordNumber,
                                  storeAttemptNo, m_connection->topics.*topic,
  ...
  boost::future<autobahn::wamp_call_result> callFuture;
  {
    const ConnectionLock lock(m_connectionMutex);
    if (!m_connection) {
      m_dropCopy->GetDataLog().Warn("no connection\t%1%\t%2%\t%3%",
                                    recordNumber, storeAttemptNo,
                                    m_connection->topics.*topic);
      return false;
    }

    try {
      callFuture = m_connection->session->call(m_connection->topics.*topic,
                                               std::make_tuple(), record,
                                               m_config.callOptions);
    } catch (const std::exception &ex) {
      GetLog().Error("Failed to call TWS to store Drop Copy record: \"%1%\".",
                     ex.what());
      m_dropCopy->GetDataLog().Error("store error\t%1%\t%2%\t%3%\t%4%",
                                     recordNumber, storeAttemptNo,
                                     m_connection->topics.*topic, ex.what());
      return false;
    }
  }
  ...
  try {
    if (callFuture.get().argument<bool>(0)) {
      m_dropCopy->GetDataLog().Info("stored\t%1%\t%2%\t%3%", recordNumber,
                                    storeAttemptNo,
                                    m_connection->topics.*topic);
    } else {
      GetLog().Error(
...
void EngineServer::Service::DumpRecord(const std::string Topics::*topic,
                                       size_t recordNumber,
                                       size_t storeAttemptNo,
                                       const DropCopyRecord &record) {
  m_dropCopy->GetDataLog().Error("dump\t%1%\t%2%\t%3%\t%4%", recordNumber,
                                 storeAttemptNo, m_connection->topics.*topic,
                                 ConvertToLogString(record));
}

bool EngineServer::Service::StoreBook(size_t /*recordNumber*/,
                                      size_t /*storeAttemptNo*/,
                                      bool dump,
                                      const Security &security,
                                      const PriceBook &book) {
  ...
  boost::future<autobahn::wamp_call_result> callFuture;
  {
    ...
    try {
      callFuture = m_connection->session->call(
          ...
          std::make_tuple(*m_instanceId, security.GetSource().GetInstanceName(),
                          security.GetSymbol().GetSymbol(),
                          ConvertToMicroseconds(book.GetTime()), book.GetBid(),
                          book.GetAsk()),
          m_config.callOptions);
    } catch (const std::exception &ex) {
      GetLog().Error(
...
Result EngineServer::Service::Request(size_t recordNumber,
                                      size_t storeAttemptNo,
                                      const std::string Topics::*topic,
                                      const DropCopyRecord &request) {
  m_dropCopy->GetDataLog().Info("request\t%1%\t%2%\t%3%\t%4%", recordNumber,
                                storeAttemptNo, topic,
  ...
  boost::future<autobahn::wamp_call_result> callFuture;
  {
    ...
    try {
      ...
      m_dropCopy->GetDataLog().Error("request error\t%1%\t%2%\t%3%\t%4%",
                                     recordNumber, storeAttemptNo,
                                     m_connection->topics.*topic, ex.what());
  ...
  try {
    ...
    m_dropCopy->GetDataLog().Info("request result\t%1%\t%2%\t%3%\t%4%",
                                  recordNumber, storeAttemptNo,
                                  m_connection->topics.*topic, result);
    return result;
  } catch (const std::exception &ex) {
    GetLog().Error("Failed to request to TWS: \"%1%\".", ex.what());
    m_dropCopy->GetDataLog().Error("request error\t%1%\t%2%\t%3%\t%4%",
...
void EngineServer::Service::PublishMessage(const char *tag,
                                           const pt::ptime &time,
  ...
  if (m_connection) {
    try {
      m_connection->session->publish(
          ...
          std::make_tuple(std::string(tag), ConvertToTimeT(time), oss.str()));
    } catch (const std::exception &ex) {
      m_log->ErrorWithoutSignal("Failed to publish log event for TWS: \"%1%\".",
                                ex.what());
    }
  } else {
