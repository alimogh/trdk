...
typedef boost::unordered_map<std::string, DropCopyRecordField> DropCopyRecord;
}
}
...
class DropCopyRecordFieldMsgpackVisitor : public boost::static_visitor<> {
 public:
  ...
  void operator()(const TimeInForce &value) { m_stream.pack(int(value)); }

  void operator()(const TradingMode &value) { m_stream.pack(int(value)); }

  void operator()(const OrderSide &value) { m_stream.pack(int(value)); }

  void operator()(const OrderStatus &value) { m_stream.pack(int(value)); }

  void operator()(const OrderType &value) { m_stream.pack(int(value)); }

  void operator()(const OperationResult &value) { m_stream.pack(int(value)); }
...
class DropCopyLogStringVisitor : public boost::static_visitor<std::string> {
 public:
  std::string operator()(
      const boost::shared_ptr<const FinancialResult> &record) const {
