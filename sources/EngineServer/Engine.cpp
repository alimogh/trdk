...
void Engine::Run(
  ...
  try {
    ...
    }
    {
      ...
      m_eventsLogFile.open(logFilePath.string().c_str(),
    ...
    m_context = boost::make_unique<trdk::Engine::Context>(
        *m_eventsLog, *m_tradingLog, settings, ini, params);
...

    {
      ...
      if (ini.ReadBoolKey("General", "trading_log")) {
        m_tradingLogFile.open(tradingLogFilePath.string().c_str(),
                              std::ios::out | std::ios::ate | std::ios::app);
        if (!m_tradingLogFile) {
        ...
        m_tradingLog->EnableStream(m_tradingLogFile, *m_context);
      } else {
    ...
    if (m_eventsLog) {
      m_eventsLog->Error("Failed to init engine context: \"%1%\".", ex.what());
    }
    ...
    try {
      ...
      if (realModuleVersion != expectedModuleVersion) {
        m_eventsLog->Error(
            ...
            ": \"%2%\", but must be \"%3%\".",
            dll.GetFile(), realModuleVersion, expectedModuleVersion);
