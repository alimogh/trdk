...
QueueService::Queue::Queue() : logicalSize(0) {}
...
bool QueueService::IsEmpty() const { return GetSize() == 0; }
...
void QueueService::Enqueue(Callback &&callback) {
  {
    const StateLock lock(m_stateMutex);
    m_currentQueue->data.emplace_back(m_nextRecordNumber, std::move(callback));
    ++m_nextRecordNumber;
    ...
    while (!m_isStopped && !IsEmpty()) {
      Queue *listToRead;
      {
        ...
        listToRead = m_currentQueue == &m_queues.first
                         ? !m_queues.second.data.empty() ? &m_queues.second
                                                         : &m_queues.first
                         : !m_queues.first.data.empty() ? &m_queues.first
                                                        : &m_queues.second;
      ...
      if (listToRead == m_currentQueue) {
        m_currentQueue = m_currentQueue == &m_queues.first ? &m_queues.second
                                                           : &m_queues.first;
      }
...

      {
          ...
          if (prevSize < listToRead->logicalSize) {
            if (++numberOfRisingIterations >= 3) {
              m_log.Warn(
                  ...
                  ": %1% -> %2% (%3%) at %4%.",
                  prevSize, listToRead->logicalSize, GetSize(),
        ...
        for (auto it = listToRead->GetLogicalBegin();
             !m_isStopped && it != listToRead->data.cend(); ++it) {
          AssertLt(0, listToRead->logicalSize);
          if (!it->Dequeue()) {
            break;
          }
...
void QueueService::OnConnectionRestored() { m_dataCondition.notify_one(); }
...
void QueueService::Dump() {
  ...
  while (!IsEmpty()) {
    Queue *const listToRead =
        m_currentQueue == &m_queues.first
            ? !m_queues.second.data.empty() ? &m_queues.second : &m_queues.first
            : !m_queues.first.data.empty() ? &m_queues.first : &m_queues.second;
    AssertLt(0, listToRead->data.size());
    AssertLt(0, listToRead->logicalSize);
    AssertGe(listToRead->data.size(), listToRead->logicalSize);
    if (listToRead == m_currentQueue) {
    ...
    Assert(!listToRead->data.empty());
    AssertGe(listToRead->data.size(), listToRead->logicalSize);
    for (auto it = listToRead->GetLogicalBegin(); it != listToRead->data.cend();
         ++it) {
      AssertLt(0, listToRead->logicalSize);
      it->Dump();
      --listToRead->logicalSize;
